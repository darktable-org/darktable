...
void dt_dev_reload_history_items(dt_develop_t *dev)
{
  ...
  dt_pthread_mutex_lock(&(darktable.db_image[dev->image_storage.id & (DT_IMAGE_DBLOCKS-1)]));
  ...
  dt_pthread_mutex_unlock(&(darktable.db_image[dev->image_storage.id & (DT_IMAGE_DBLOCKS-1)]));
}
