...
static void _blendop_masks_mode_callback(const unsigned int mask_mode, dt_iop_gui_blend_data_t *data)
{
         ...
         || (data->blendif_inited && (mask_mode & DEVELOP_MASK_CONDITIONAL))))
  {
    ...
    if(data->module->blend_colorspace(data->module, NULL, NULL) == iop_cs_RAW)
    {
      ...
      gtk_widget_set_sensitive(data->local_contrast_slider, FALSE);
      gtk_widget_hide(GTK_WIDGET(data->local_contrast_slider));
    }
    else
    {
...
static void _blendop_blendif_local_contrast_callback(GtkWidget *slider, dt_iop_gui_blend_data_t *data)
{
  if(darktable.gui->reset || !data || !data->blendif_inited) return;
  dt_develop_blend_params_t *bp = data->module->blend_params;
  const float oldval = bp->local_contrast;
  bp->local_contrast = dt_bauhaus_slider_get(slider);
  dt_dev_add_history_item(darktable.develop, data->module, TRUE);

  if((oldval == 0.0f) && (bp->local_contrast != 0.0f))
  {
    dt_dev_reprocess_all(data->module->dev);
    dt_control_queue_redraw();
  }
...
static gboolean _blendop_blendif_reset(GtkButton *button, GdkEventButton *event, dt_iop_module_t *module)
{
  ...
  module->blend_params->local_contrast = module->default_blendop_params->local_contrast;
...
void dt_iop_gui_update_blending(dt_iop_module_t *module)
{
  ...
  dt_bauhaus_slider_set(bd->local_contrast_slider, module->blend_params->local_contrast);
  ...
  gtk_widget_set_visible(bd->local_contrast_slider, dt_image_is_rawprepare_supported(&img));
...
void dt_iop_gui_init_blending(GtkWidget *iopw, dt_iop_module_t *module)
{
  ...
  if(module->flags() & IOP_FLAGS_SUPPORTS_BLENDING)
  {
    ...
    bd->local_contrast_slider = dt_bauhaus_slider_new_with_range(module, -1.0f, 1.0f, .01f, 0.0f, 2);
    dt_bauhaus_widget_set_label(bd->local_contrast_slider, N_("local contrast threshold"), N_("local contrast threshold"));
    dt_bauhaus_slider_set_format(bd->local_contrast_slider, _("%.2f"));
    gtk_widget_set_tooltip_text(bd->local_contrast_slider, _("adjust the threshold for the local contrast mask.\npositive values select for high local contrast\nnegative values select flat areas"));
    g_signal_connect(G_OBJECT(bd->local_contrast_slider), "value-changed", G_CALLBACK(_blendop_blendif_local_contrast_callback), bd);
    ...
    gtk_box_pack_start(GTK_BOX(bd->bottom_box), bd->local_contrast_slider, TRUE, TRUE, 0);
