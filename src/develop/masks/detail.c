...
static void _masks_blur_5x5_coeff(float *c,
                           const float sigma)
{
  float kernel[5][5];
...
static void _masks_blur_9x9_coeff(float *c, const float sigma)
{
  float kernel[9][9];
  const float temp = -2.0f * sqrf(sigma);
                       ...
                       const float clip)
{
  ...
  schedule(simd:static) aligned(src, out : 64)
    ...
    dt_masks_extend_border(out, width, height, 2);
    return;
  }

  _masks_blur_9x9_coeff(blurmat, sigma);
#ifdef _OPENMP
  #pragma omp parallel for simd default(none) \
                               ...
                               const gboolean output)
{
  ...
  dt_masks_blur(tmp, mask, details->roi.width, details->roi.height, sigma, 1.0f, 1.0f);
  dt_free_align(tmp);
