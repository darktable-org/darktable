...
static inline void _gradient_point_transform(const float xref, const float yref, const float x, const float y, const float sinv, const float cosv,
...
static void dt_gradient_get_distance(float x, float y, float as, dt_masks_form_gui_t *gui, int index, int *inside, int *inside_border, int *near, int *inside_source)
{
  if (!gui) return;
  ...
  *near = -1;
  ...
  {
    if((x - gpt->border[i*2])*(x - gpt->border[i*2])+(y - gpt->border[i*2+1])*(y - gpt->border[i*2+1]) < as2)
    {
      *inside_border = 1;
      return;
    }
  ...
  {
    if((x - gpt->points[i*2])*(x - gpt->points[i*2])+(y - gpt->points[i*2+1])*(y - gpt->points[i*2+1]) < as2)
    {
      *inside = 1;
      return;
    }
...
static int dt_gradient_events_mouse_scrolled(struct dt_iop_module_t *module, float pzx, float pzy, int up, uint32_t state,
    dt_masks_form_t *form, int parentid, dt_masks_form_gui_t *gui, int index)
{
  if (gui->form_selected)
  {
    ...
    else if(gui->edit_mode == DT_MASKS_EDIT_FULL)
    {
      ...
      if(up) gradient->compression = fmaxf(gradient->compression, 0.001f) * 0.8f;
      else   gradient->compression = fminf(fmaxf(gradient->compression, 0.001f) * 1.0f/0.8f, 1.0f);
    ...
    dt_masks_form_t *form, int parentid, dt_masks_form_gui_t *gui, int index)
{
  ...
  else if (gui->creation)
  {
    ...
    const float steepness = 0.0f; //MIN(1.0f,dt_conf_get_float("plugins/darkroom/masks/gradient/steepness")); // currently not used
...
static void dt_gradient_events_post_expose(cairo_t *cr, float zoom_scale, dt_masks_form_gui_t *gui, int index)
{
  ...
  if (gpt->points_count > 4)
  {
    ...
    {
      _gradient_point_transform(xref, yref, gpt->points[i*2]+dx, gpt->points[i*2+1]+dy, sinv, cosv, &x, &y);
  ...
  if ((gui->group_selected == index) && gpt->border_count > 3)
  {
    ...
    while(count < border_count)
    {
      ...
      _gradient_point_transform(xref, yref, gpt->border[count*2]+dx, gpt->border[count*2+1]+dy, sinv, cosv, &x, &y);
      cairo_move_to(cr, x, y);
      count++;
...
static int dt_gradient_get_points_border(dt_develop_t *dev, float x, float y, float rotation, float distance, float **points, int *points_count)
{
  *points = NULL;
  *points_count = 0;
...
static int dt_gradient_get_area(dt_iop_module_t *module, dt_dev_pixelpipe_iop_t *piece, dt_masks_form_t *form, int *width, int *height, int *posx, int *posy)
{
...
static int dt_gradient_get_mask(dt_iop_module_t *module, dt_dev_pixelpipe_iop_t *piece, dt_masks_form_t *form, float **buffer, int *width, int *height, int *posx, int *posy)
{
  ...
  if (darktable.unmuted & DT_DEBUG_PERF) dt_print(DT_DEBUG_MASKS, "[masks %s] gradient area took %0.04f sec\n", form->name, dt_get_wtime()-start2);
  start2 = dt_get_wtime();
