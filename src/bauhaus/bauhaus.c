...
static void dt_bauhaus_style_updated(GtkWidget *widget, gpointer user_data)
{
  ...
  if(w->type == DT_BAUHAUS_COMBOBOX)
  {
    gtk_widget_set_size_request(widget, -1,
                                margin.top + padding.top + margin.bottom + padding.bottom + get_line_height());
  }
  else if(w->type == DT_BAUHAUS_SLIDER)
  {
    gtk_widget_set_size_request(widget, -1,
                                margin.top + padding.top + margin.bottom + padding.bottom + INNER_PADDING
                                    + darktable.bauhaus->baseline_size + get_line_height()
  ...
  switch(w->type)
  {
    case DT_BAUHAUS_SLIDER:
    {
      ...
      for(int k = 0; k < num_scales; k++)
      {
        ...
        draw_slider_line(cr, d->oldpos, off, scale, width, height, ht);
        cairo_stroke(cr);
      }
      cairo_restore(cr);
      set_color(cr, *fg_color);
      ...
      const float mouse_off
          = darktable.bauhaus->change_active
                ? get_slider_line_offset(d->oldpos, scale, (darktable.bauhaus->mouse_x - INNER_PADDING)/ width,
                                         darktable.bauhaus->mouse_y / height, ht / (float)height, width)
                : 0.0f;
      draw_slider_line(cr, d->oldpos, mouse_off, scale, width, height, ht);
      cairo_stroke(cr);
      cairo_restore(cr);
      ...
      show_pango_text(w, context, cr, max, wd - darktable.bauhaus->quad_width - INNER_PADDING, ht + INNER_PADDING,
      ...
      for(int j = 0; j < d->entries->len; j++)
      {
        ...
        if(!strncmp(text_cmp, keys, darktable.bauhaus->keys_cnt))
        {
          ...
          if(entry->alignment == DT_BAUHAUS_COMBOBOX_ALIGN_LEFT)
          {
            ...
            label_width = show_pango_text(w, context, cr, entry->label, wd - darktable.bauhaus->quad_width,
...
static gboolean _widget_draw(GtkWidget *widget, cairo_t *crf)
{
  ...
  // translate to account for the widget spacing
  const int h2 = height - margin.top - margin.bottom;
  const int w2 = width - margin.left - margin.right;
  const int h3 = h2 - padding.top - padding.bottom;
  const int w3 = w2 - padding.left - padding.right;
  gtk_render_background(context, cr, margin.left, margin.top, w2, h2);
  cairo_translate(cr, margin.left + padding.left, margin.top + padding.top);
  ...
  gtk_render_frame(context, crf, margin.left, margin.top, w2, h2);
...
static gboolean dt_bauhaus_slider_button_press(GtkWidget *widget, GdkEventButton *event, gpointer user_data)
{
  ...
  else if(event->button == 1)
  {
    ...
    else
    {
      ...
      else if(event->y > darktable.bauhaus->line_height)
      {
        const float l = 0.0f;
...
static gboolean dt_bauhaus_slider_motion_notify(GtkWidget *widget, GdkEventMotion *event, gpointer user_data)
{
  ...
  if(d->is_dragging && event->state & GDK_BUTTON1_MASK)
  {
    ...
    {
      const float scaled_step = allocation.width * (r - l) * dt_bauhaus_slider_get_step(widget) / (d->max - d->min);
      const float steps = floorf((event->x - darktable.bauhaus->mouse_x) / scaled_step);
