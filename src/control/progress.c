...
static void _global_progress_start(dt_progress_t *progress)
{
...
static void _global_progress_set(dt_progress_t *progress, double value)
{
  dt_control_t *control = darktable.control;
...
static void _global_progress_end(dt_progress_t *progress)
{
  dt_control_t *control = darktable.control;
  control->progress_system.n_progress_bar--;
...
dt_progress_t *dt_control_progress_create(const gboolean has_progress_bar, const gchar *message)
{
  dt_control_t *control = darktable.control;
  // create the object
  ...
  if(has_progress_bar) _global_progress_start(progress);
  ...
  dt_print(DT_DEBUG_CONTROL | DT_DEBUG_LUA, "[dt_control_progress_create] progress at %p", progress);
  return progress;
}
...
void dt_control_progress_destroy(dt_progress_t *progress)
{
  ...
  control->progress_system.list_length--;
  if(progress->has_progress_bar) _global_progress_end(progress);
  ...
  dt_print(DT_DEBUG_CONTROL | DT_DEBUG_LUA, "[dt_control_progress_destroy] progress at %p", progress);
  free(progress);
}
                                          ...
                                          void *data)
{
  ...
  if(control->progress_system.proxy.module != NULL)
    control->progress_system.proxy.cancellable(control->progress_system.proxy.module, progress->gui_data,
                                               progress);
  dt_pthread_mutex_unlock(&control->progress_system.mutex);
  dt_print(DT_DEBUG_CONTROL | DT_DEBUG_LUA, "[dt_control_progress_make_cancellable] progress at %p", progress);
}

static void _control_progress_cancel_callback(dt_progress_t *progress, void *data)
{
  // cancelling a job always includes destroying the progress structure including the mutex
...
void dt_control_progress_cancel(dt_progress_t *progress)
{
  ...
  dt_pthread_mutex_lock(&progress->mutex);
  const gboolean ignore = progress->cancel == NULL;
  dt_pthread_mutex_unlock(&progress->mutex);
  if(ignore) return;
...
/*  Calling the progress->cancel callback is somewhat tricky as that might include
    ...
    _control_progress_cancel_callback() is doing so as it deep-down calls dt_control_job_cancel().

    In that case we must not call progress->cancel with the mutex locked as that would make
    the mutex destroy failing leading to assert() failures in debug builds and we can't later
    use that mutex anyway.
    Otherwise do the cancel callback in mutex locked state.
*/
  const gboolean destroying = progress->cancel == &_control_progress_cancel_callback;
  if(destroying)
  ...
  dt_print(DT_DEBUG_CONTROL | DT_DEBUG_LUA, "[dt_control_progress_cancel] %s progress at %p",
    destroying ? "destroyed" : "kept", progress);
}
...
void dt_control_progress_set_progress(dt_progress_t *progress, double value)
{
  ...
  if(progress->has_progress_bar) _global_progress_set(progress, value);
