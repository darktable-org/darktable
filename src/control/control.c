...
time_t dt_diratime_action(const char *dir_path, const char *action)
{
  ...
  if((
       (!g_strcmp0(action, "update") && (!g_strcmp0(ext, ".xmp") || !g_strcmp0(ext, ".XMP")))
  ...
  if(!g_strcmp0(action, "create"))
  {
    ...
    {
      GFileOutputStream *out = g_file_replace(_g_dir_mark,
                                              NULL,
                                              FALSE,
                                              ...
                                              &error);
      g_object_unref(out);
      g_file_set_attribute_uint64(_g_dir_mark,
                                  G_FILE_ATTRIBUTE_TIME_MODIFIED,
                                  dir_mark_time,
                                  G_FILE_QUERY_INFO_NOFOLLOW_SYMLINKS,
                                  FALSE,
  ...
  {
    if(!g_file_query_exists(_g_dir_mark, NULL))
    {
    GFileOutputStream *out = g_file_replace(_g_dir_mark,
                                            NULL,
                                            FALSE,
