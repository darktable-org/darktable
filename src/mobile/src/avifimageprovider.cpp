                                        ...
                                        const QSize &requestedSize)
{
    ...
    if (!preview.isNull()) {
        // Both thumbnail and full-preview JEPGs are tagged sRGB by the daemon.
        // Ensure Qt always sees a valid colorspace; convert if anything exotic
        // sneaks through (e.g. an AdobeRGB or Display-P3 JPEG shared directly).
        if (!preview.colorSpace().isValid())
            preview.setColorSpace(QColorSpace(QColorSpace::SRgb));
        else if (preview.colorSpace() != QColorSpace(QColorSpace::SRgb))
