                                        ...
                                        const QSize &requestedSize)
{
    ...
    if (!preview.isNull()) {
        // Ensure the image always carries an explicit color space so Qt's
        // renderer applies the correct transform on the GPU.
        //
        // darktable's mipmap cache stores display-referred sRGB JPEG bytes
        // without an APP2 ICC marker. Tag them as sRGB (no pixel conversion
        // needed — the values are already correct).
        //
        // For full-size previews generated by darktable-cli the ICC profile is
        // embedded; Qt reads it automatically. If the embedded profile is not
        // sRGB (e.g. AdobeRGB, Display P3), convert the pixel values so the
        // image renders accurately on every Android display.
