...
dt_dlopencl_t *dt_dlopencl_init(const char *name)
{
  ...
  if(module == NULL)
  {
    dt_print_nts(DT_DEBUG_ALWAYS, "[dt_dlopencl_init] could not find any opencl runtime library\n");
    return NULL;
  }
  ...
  if(ocl->symbols == NULL)
  {
    dt_print_nts(DT_DEBUG_ALWAYS, "[dt_dlopencl_init] could not find symbols in opencl runtime library\n");
    free(ocl);
    free(module);
    return NULL;
  }
