...
static inline cl_int _migrate_cl_mem(const int devid, cl_mem mem)
{
                                                 ...
                                                 const int overlap)
{
  ...
  if(cl->dev[devid].max_image_width < width
      ...
      || _opencl_get_device_memalloc(devid) < MAX(iplane, oplane))
