                                     ...
                                     gboolean *loaded_cached)
{
  ...
  *loaded_cached = FALSE;
  ...
  if(rd != filesize)
  {
    free(file);
    dt_print_nts(DT_DEBUG_OPENCL,
             "[opencl_load_source] could not read all of file `%s' for program number %d\n",
      filename, prog);
    return FALSE;
  }
                                      ...
                                      const gboolean loaded_cached)
{
  ...
  (cl->dlocl->symbols->dt_clGetProgramBuildInfo)(program, cl->dev[dev].devid,
                                                 ...
                                                 &ret_val_size);
  if(ret_val_size > 1 && ret_val_size < 0x100000) // we don't want to see nothing or crazy things 
  {
                              ...
                              const void *arg)
{
  ...
  if(err != CL_SUCCESS)
    dt_print(DT_DEBUG_OPENCL,
             "[opencl_set_kernel_arg] error kernel `%s' (%i) on device `%s' id=%d: %s",
              darktable.opencl->name_saved[kernel], kernel, cl->dev[dev].fullname, dev, cl_errstr(err));
  return err;
}
