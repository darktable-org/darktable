...
static gpointer _fetch_worker(gpointer data)
{
  ...
  if(_cancelled())
  {
    /* Set a terminal status before leaving. The out: block clears the cancel
       flag but does not touch the state, and a fetch left reading RUNNING
       stays that way for the session: sf_fetch_start() refuses to start while
       it holds, the module's only download control stays a cancel button, and
       the status row keeps re-arming its poll timer. This is the ordinary
       landing place for an early cancel, since the manifest GET installs no
       progress callback and cannot be aborted mid-transfer. */
