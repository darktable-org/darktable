                                ...
                                const dt_iop_order_iccprofile_info_t *const profile)
{
  ...
  DT_OMP_FOR(if (bsize > 4) reduction(+ : acc[:4]) reduction(min : low[:4]) reduction(max : high[:4]))
  for(size_t y = box[1]; y < box[3]; y++)
  {
    for(size_t x = box[0]; x < box[2]; x++)
    {
      ...
      dt_XYZ_D65_2_XYZ_D50(source + (y * roi->width + x) * 4, XYZ_D50);
      cmsDoTransform(xform, XYZ_D50, pLAB, 1);
