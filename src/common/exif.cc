         ...
         (param=xmpData.findKey(Exiv2::XmpKey("Xmp.darktable.history_params")))   != xmpData.end() )
    {
      ...
      if(cnt == en->count() && cnt == op->count() && cnt == param->count())
      {
        ...
        for(int i=0; i<cnt; i++)
        {
          ...
          int blendop_size = 0;
          std::string blendop_str;
          if(blendop != xmpData.end() && blendop->size() > 0 && blendop->count () > i && (blendop_str = blendop->toString(i)).c_str() != NULL)
          {
          ...
          std::string multi_name_str;
          if(multi_name != xmpData.end() && multi_name->size() > 0 &&
	     multi_name->count() > i &&
	     (multi_name_str = multi_name->toString(i)).c_str() != NULL)
          {
