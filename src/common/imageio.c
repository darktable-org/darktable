                         ...
                         dt_export_metadata_t *metadata, const int history_end)
{
  ...
  {
    fprintf(stderr, "[dt_imageio_snapshots] mipmap allocation for `%s' failed\n", filename);
  ...
  gboolean res =
    dt_dev_pixelpipe_init_export(&pipe, wd, ht, format->levels(format_params), FALSE);

  if(!res)
  {
    dt_control_log(
      _("failed to allocate memory for snapshot, please lower the threads used for export or buy more memory."));
    goto error;
  }

  const gboolean use_style = format_params->style[0] != '\0';
  ...
  {
    fprintf(stderr,"[dt_imageio_snapshots] ");
    if(use_style)
    {
  ...
  dt_dev_pixelpipe_get_dimensions
    (&pipe, &dev, pipe.iwidth, pipe.iheight,
  ...
  if(dt_dev_distort_backtransform_plus(&dev, &pipe, 0.f, DT_DEV_TRANSFORM_DIR_ALL, origin, 1))
  {
    ...
    processed_width = scale * pipe.processed_width;
    processed_height = scale * pipe.processed_height;

    dt_print(DT_DEBUG_IMAGEIO,"[dt_imageio_snapshots] imgid %d, pipe %ix%i, range %ix%i --> exact %i, upscale %i, hq %i, corrected %i, scale %.7f, corr %.6f, size %ix%i\n",
             imgid, pipe.processed_width, pipe.processed_height, format_params->max_width, format_params->max_height,
             exact_size, upscale, FALSE, corrected, scale, corrscale, processed_width, processed_height);
  }
  else
  {
    processed_width = floor(scale * pipe.processed_width);
    processed_height = floor(scale * pipe.processed_height);
    dt_print(DT_DEBUG_IMAGEIO,"[dt_imageio_snapshots] (direct) imgid %d, hq %i, pipe %ix%i, range %ix%i --> size %ix%i / %ix%i\n",
             imgid, FALSE, pipe.processed_width, pipe.processed_height, format_params->max_width, format_params->max_height,
  ...
  dt_dev_pixelpipe_process(&pipe, &dev, format_params->x, format_params->y,
                           processed_width - format_params->x, processed_height - format_params->y, scale);

  if(finalscale) finalscale->enabled = 1;

  dt_show_times(&start, "[dev_process_export] pixel pipeline processing");

  uint8_t *outbuf = pipe.backbuf;
  if(outbuf == NULL)
  {
    dt_print(DT_DEBUG_IMAGEIO, "[dt_imageio_snapshots] no valid output buffer\n");
    goto error;
  }
