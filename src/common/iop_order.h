...
/** returns the kind of the list by looking at the order of the modules, it is either one of the built-in version
    or a customr order  */
dt_iop_order_t dt_ioppr_get_iop_order_list_kind(GList *const iop_order_list);

/** returns a list of dt_iop_order_entry_t and updates *_version */
GList *dt_ioppr_get_iop_order_list(int32_t imgid, const gboolean sorted);
...
dt_iop_order_entry_t *dt_ioppr_get_iop_order_entry(GList *const iop_order_list, const char *const op_name, const int multi_priority);
/** likewise, but returns the link in the list instead of the entry */
GList *dt_ioppr_get_iop_order_link(GList *const op_order_list, const char *op_name, const int multi_priority);

/** returns the iop_order from iop_order_list list with operation = op_name */
int dt_ioppr_get_iop_order(GList *const iop_order_list, const char *const op_name, const int multi_priority);
/** returns TRUE if operation/multi-priority is before base_operation (first in pipe) on the iop-list */
gboolean dt_ioppr_is_iop_before(GList *const iop_order_list, const char *const base_operation,
                                const char *const operation, const int multi_priority);
/* write iop-order list for the given image */
gboolean dt_ioppr_write_iop_order_list(GList *const iop_order_list, const int32_t imgid);
gboolean dt_ioppr_write_iop_order(const dt_iop_order_t kind, GList *const iop_order_list, const int32_t imgid);

/** serialize list, used for presets */
void *dt_ioppr_serialize_iop_order_list(GList *const iop_order_list, size_t *size);
...
char *dt_ioppr_serialize_text_iop_order_list(GList *const iop_order_list);
GList *dt_ioppr_deserialize_text_iop_order_list(const char *buf);

/** insert a match for module into the iop-order list */
void dt_ioppr_insert_module_instance(struct dt_develop_t *const dev, struct dt_iop_module_t *const module);
void dt_ioppr_resync_modules_order(struct dt_develop_t *const dev);
void dt_ioppr_resync_iop_list(struct dt_develop_t *const dev);
...
void dt_ioppr_update_for_entries(struct dt_develop_t *const dev, GList *const entry_list, gboolean append);
void dt_ioppr_update_for_style_items(struct dt_develop_t *const dev, GList *const st_items, gboolean append);
void dt_ioppr_update_for_modules(struct dt_develop_t *const dev, GList *const modules, gboolean append);

/** check if there's duplicate iop_order entries in iop_list */
void dt_ioppr_check_duplicate_iop_order(GList **_iop_list, GList *const history_list);

/** sets the default iop_order to iop_list */
void dt_ioppr_set_default_iop_order(struct dt_develop_t *const dev, const int32_t imgid);
void dt_ioppr_migrate_iop_order(struct dt_develop_t *const dev, const int32_t imgid);
void dt_ioppr_change_iop_order(struct dt_develop_t *const dev, const int32_t imgid, GList *const new_iop_list);
/** extract all modules with multi-instances */
GList *dt_ioppr_extract_multi_instances_list(GList *const iop_order_list);
/** merge all modules with multi-instances as extracted with routine above into a canonical iop-order list */
GList *dt_ioppr_merge_multi_instance_iop_order_list(GList *const __restrict__ iop_order_list, GList *const __restrict__ multi_instance_list);

/** returns 1 if there's a module_so without a iop_order defined */
int dt_ioppr_check_so_iop_order(GList *const __restrict__ iop_list, GList *const __restrict__ iop_order_list);
...
GList *dt_ioppr_iop_order_copy_deep(GList *const iop_order_list);
...
gboolean dt_ioppr_check_can_move_before_iop(GList *const iop_list, struct dt_iop_module_t *const __restrict__module, struct dt_iop_module_t *const __restrict__ module_next);
/** returns the iop_order after module_prev if module can be moved */
gboolean dt_ioppr_check_can_move_after_iop(GList *const iop_list, struct dt_iop_module_t *const __restrict__ module, struct dt_iop_module_t *const __restrict__ module_prev);

/** moves module before/after module_next/previous on pipe */
gboolean dt_ioppr_move_iop_before(struct dt_develop_t *const dev, struct dt_iop_module_t *const __restrict__ module, struct dt_iop_module_t *const __restrict__ module_next);
gboolean dt_ioppr_move_iop_after(struct dt_develop_t *const dev, struct dt_iop_module_t *const __restrict__ module, struct dt_iop_module_t *const __restrict__ module_prev);

// for debug only
int dt_ioppr_check_iop_order(struct dt_develop_t *const dev, const int imgid, const char *msg);
void dt_ioppr_print_module_iop_order(GList *const iop_list, const char *msg);
void dt_ioppr_print_history_iop_order(GList *const history_list, const char *msg);
void dt_ioppr_print_iop_order(GList *const iop_order_list, const char *msg);
