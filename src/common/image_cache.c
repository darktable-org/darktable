...
dt_image_cache_allocate(void *data, const uint32_t key, int32_t *cost, void **buf)
{
  ...
  *cost = sizeof(dt_image_t);
  ...
  char *str;
  sqlite3_stmt *stmt;
  DT_DEBUG_SQLITE3_PREPARE_V2(dt_database_get(darktable.db), "select id, group_id, film_id, width, height, filename, maker, model, lens, exposure, aperture, iso, focal_length, datetime_taken, flags, crop, orientation, focus_distance, raw_parameters, longitude, latitude, color_matrix, colorspace, sub_id from images where id = ?1", -1, &stmt, NULL);
  ...
  if(sqlite3_step(stmt) == SQLITE_ROW)
  {
    ...
    img->sub_id = sqlite3_column_int(stmt, 23);
  ...
  dt_image_cache_write_mode_t mode)
{
  ...
  DT_DEBUG_SQLITE3_PREPARE_V2(dt_database_get(darktable.db),
                              ...
                              "latitude = ?19, color_matrix = ?20, colorspace = ?21, sub_id = ?22 where id = ?23", -1, &stmt, NULL);
  ...
  DT_DEBUG_SQLITE3_BIND_INT(stmt, 22, img->sub_id);
