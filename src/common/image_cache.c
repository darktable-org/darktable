...
void dt_image_cache_write_release(dt_image_cache_t *cache, dt_image_t *img, dt_image_cache_write_mode_t mode)
{
  ...
  DT_DEBUG_SQLITE3_BIND_INT(stmt, 17, *(uint32_t *)(&img->legacy_flip));
  DT_DEBUG_SQLITE3_BIND_INT(stmt, 18, img->group_id);
  DT_DEBUG_SQLITE3_BIND_DOUBLE(stmt, 19, img->longitude);
  DT_DEBUG_SQLITE3_BIND_DOUBLE(stmt, 20, img->latitude);
  DT_DEBUG_SQLITE3_BIND_DOUBLE(stmt, 21, img->elevation);
