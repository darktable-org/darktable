...
gboolean _cache_update_refactored(const gboolean only_visible, const gboolean force, const gboolean ordered)
{
  /** Here's how it works
   *
   * Priority is:
   * Selection > Mouseover > Active Images
   *
   * Culling Dynamic has a different order:
   * Mouseover > Active Images
   *
   * Fullscreen Views:
  ...
  // and every image in the subset is always selected.
  // And since we always have at least 1 active image in culling dynamic, and the active
  ...
  else if(lightroom_view && dt_view_lighttable_get_layout(darktable.view_manager) == DT_LIGHTTABLE_LAYOUT_CULLING_DYNAMIC)
  {
    if(mouseover > 0)
    {
      // we only allow mouseover on active images
...
// cache the list of images to act on during global changes (libs, accels)
// return TRUE if the cache is updated, FALSE if it's still up to date
gboolean _cache_update_legacy(const gboolean only_visible, const gboolean force, const gboolean ordered)
{
  /** Here's how it works
   *
   *             mouse over| x | x | x |   |   |
...
{
  GList *l = dt_act_on_get_images(only_visible, FALSE, TRUE);
...
int dt_act_on_get_main_image_refactored(const gboolean prioritize_hover)
{
   ...
   *
   * Priority is:
   *          (Mouseover >)           Selection > Active Images > Mouseover
   * (if prioritize_hover == TRUE)
   *
   *
   **/
