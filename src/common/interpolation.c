                                          ...
                                          const dt_iop_roi_t *const roi_in)
{
  ...
  if(roi_out->scale == 1.f)
  {
    ...
    DT_OMP_FOR_CLAUSE(shared(out),in, in_stride_floats, out_stride_floats, roi_out, x0)
  ...
  DT_OMP_FOR_CLAUSE(shared(vmeta),
                    in, in_stride_floats, out, out_stride_floats, height, width,
                                             ...
                                             const dt_iop_roi_t *const roi_in)
{
  ...
  if(roi_out->scale == 1.f)
  {
    ...
    DT_OMP_FOR_CLAUSE(shared(out), in, in_stride, out_stride, roi_out, x0)
    for(int y = 0; y < roi_out->height; y++)
    {
  ...
  // Process each output line
  DT_OMP_FOR_CLAUSE(shared(out, hindex, hlength, hkernel, vindex, vlength, vkernel, vmeta),
                    in, in_stride, out_stride, roi_out)
