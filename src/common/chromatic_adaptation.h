...
static inline void convert_XYZ_to_bradford_LMS(const dt_aligned_pixel_t XYZ, dt_aligned_pixel_t LMS)
{
  ...
  dt_apply_transposed_color_matrix(XYZ, XYZ_to_Bradford_LMS_transposed, LMS);
}
...
static inline void convert_bradford_LMS_to_XYZ(const dt_aligned_pixel_t LMS, dt_aligned_pixel_t XYZ)
{
  ...
  dt_apply_transposed_color_matrix(LMS, Bradford_LMS_to_XYZ_transposed, XYZ);
}
...
{
  // Warning : needs XYZ normalized with Y - you need to downscale before
  dt_apply_transposed_color_matrix(XYZ, XYZ_to_CAT16_LMS_transposed, LMS);
}
...
{
  // Warning : output XYZ normalized with Y - you need to upscale later
  dt_apply_transposed_color_matrix(LMS, CAT16_LMS_to_XYZ_transposed, XYZ);
}
...
/* Pre-solved matrices to adjust white point for triplets in CIE XYZ 1931 2° observer.
   They are transposed to allow more efficient SIMD calculations. */
...
const dt_colormatrix_t XYZ_D50_to_D65_Bradford_transposed
...
const dt_colormatrix_t XYZ_D65_to_D50_Bradford_transposed
    = { {  1.04792979f,  0.02962781f, -0.00924304f, 0.f },
        {  0.02294687f,  0.99043443f,  0.01505519f, 0.f },
        { -0.05019227f, -0.01707380f,  0.75187428f, 0.f } };
