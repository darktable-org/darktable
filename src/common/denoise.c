    ...
    float       *tmp)
{
  ...
  for(int kj=-K;kj<height - prior_height + K;kj++)
  {
    for(int ki=-K;ki<width - prior_width + K;ki++)
    {
      ...
      // determine relevant pixels:
      const int rel_i = CLAMP(pvx - K, 0, width),  rel_I = CLAMP(pvx + K, 0, width);
      const int rel_j = CLAMP(pvy - K, 0, height), rel_J = CLAMP(pvy + K, 0, height);
#ifdef _OPENMP
      ...
      for(int j=rel_j; j<rel_J; j++)
      {
        ...
        if(!inited_slide)
        {
          ...
          for(int jj=-Pm;jj<=PM;jj++)
          {
            ...
            // FIXME: *scale{x,y} to convert to prior space!
