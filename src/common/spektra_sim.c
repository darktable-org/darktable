...
struct sf_sim_t
{
  ...
  /* per-sublayer absolute (fog-inclusive) Dmax and particle count, precomputed
     ...
     (see spektra_sim.c's _sf_build_grain_layers). */
...
/* same Mitchell gather as cubic_interp_2d(), over a float source with an
   ...
   somewhere other than its stored cell, and normalizing on the stored cell
   would leave the reconstructed neutral off gray by whatever the difference
   is */
                       ...
                       char **errmsg)
{
  ...
  /* [su] compute_tc_lut: spectra × (sensitivity × window / norm) for an
     irradiance table; relight × sensitivity, neutral-normalized, for a
     reflectance one */
...
#pragma omp parallel for schedule(static)
    ...
    /* [su] compute_reflectance_tc_lut, white balance: send the table's OWN
       ...
       recovered neutral is not perfectly flat still resolves to gray. This is
...
#pragma omp parallel for schedule(static)
      ...
      for(int i = 0; i < n; i++)
        for(int j = 0; j < n; j++)
        {
          ...
          quad2tri(xy, tc);
          /* centered on the white the runtime PROJECTS under, which for a
             ...
             neutral off the cell the normalization above just fixed */
  ...
  {
    /* spektrafilm's single fixed grain profile, used for any stock the pack
