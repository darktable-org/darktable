...
static void _slider_class_init(GtkDarktableSliderClass *klass)
{
  ...
  _slider_signals[VALUE_CHANGED] = g_signal_new("value-changed", G_TYPE_FROM_CLASS(klass), G_SIGNAL_RUN_LAST,
...
static void _slider_init(GtkDarktableSlider *slider)
{
  ...
  gtk_alignment_set_padding(GTK_ALIGNMENT(alignment), 0, 0, DTGTK_SLIDER_BORDER_WIDTH * 2,
                            DTGTK_SLIDER_ADJUST_BUTTON_WIDTH + DTGTK_SLIDER_BORDER_WIDTH * 2);
  ...
  g_signal_connect(G_OBJECT(slider->entry), "key-press-event", G_CALLBACK(_slider_entry_key_event),
                   (gpointer)slider);
...
static gboolean _slider_entry_key_event(GtkWidget *widget, GdkEventKey *event, gpointer data)
{
  ...
  if(event->keyval == GDK_KEY_Escape || event->keyval == GDK_KEY_Tab)
    _slider_entry_abort(DTGTK_SLIDER(data));
      ...
      event->keyval == GDK_KEY_minus || event->keyval == GDK_KEY_KP_Subtract || event->keyval == GDK_KEY_plus
      || event->keyval == GDK_KEY_KP_Add || event->keyval == GDK_KEY_period
      || event->keyval == GDK_KEY_KP_Decimal || event->keyval == GDK_KEY_Left
      || event->keyval == GDK_KEY_Right || event->keyval == GDK_KEY_Delete
      || event->keyval == GDK_KEY_BackSpace || event->keyval == GDK_KEY_0 || event->keyval == GDK_KEY_KP_0
      || event->keyval == GDK_KEY_1 || event->keyval == GDK_KEY_KP_1 || event->keyval == GDK_KEY_2
      || event->keyval == GDK_KEY_KP_2 || event->keyval == GDK_KEY_3 || event->keyval == GDK_KEY_KP_3
      || event->keyval == GDK_KEY_4 || event->keyval == GDK_KEY_KP_4 || event->keyval == GDK_KEY_5
      || event->keyval == GDK_KEY_KP_5 || event->keyval == GDK_KEY_6 || event->keyval == GDK_KEY_KP_6
      || event->keyval == GDK_KEY_7 || event->keyval == GDK_KEY_KP_7 || event->keyval == GDK_KEY_8
      || event->keyval == GDK_KEY_KP_8 || event->keyval == GDK_KEY_9 || event->keyval == GDK_KEY_KP_9)
  {
    return FALSE;
  }
...
// helper function to render a filled rectangle with rounded corners, type defined if its ctrl background or
// value
static void _slider_draw_rounded_rect(cairo_t *cr, gfloat x, gfloat y, gfloat width, gfloat height,
                                      gfloat radius, int type)
{
...
static gboolean _slider_draw(GtkWidget *widget, cairo_t *cr)
{
  ...
  gfloat vscale
      = (value - gtk_adjustment_get_lower(slider->adjustment))
        / (gtk_adjustment_get_upper(slider->adjustment) - gtk_adjustment_get_lower(slider->adjustment));


  cairo_set_source_rgba(cr, (style->fg[state].red / 65535.0) * 1.7, (style->fg[state].green / 65535.0) * 1.7,
                        (style->fg[state].blue / 65535.0) * 1.7, 0.2);
  ...
  /* Draw label */
  cairo_set_source_rgba(cr, (style->text[state].red / 65535.0) * 1.7,
                        (style->text[state].green / 65535.0) * 1.7, (style->text[state].blue / 65535.0) * 1.7,
                        0.8);
  ...
  if(label)
  {
    ...
    /* store the size of the label. doing it in dtgtk_slider_set_label doesn't work as the widget isn't
     * created yet. */
    if(slider->labelwidth == 0 && slider->labelheight == 0)
    {
      cairo_text_extents(cr, label, &ext);
      slider->labelwidth = vr.x + (DTGTK_SLIDER_BORDER_WIDTH * 2) + ext.width
                           + (DTGTK_SLIDER_BORDER_WIDTH * 2);
  ...
  if(unit)
  {
    cairo_move_to(cr, vr.x + vr.width - ext.width - (DTGTK_SLIDER_BORDER_WIDTH),
                  vr.y + vr.height - (DTGTK_SLIDER_BORDER_WIDTH * 2));
  ...
  cairo_move_to(cr, vr.x + vr.width - ext.width - unitwidth - (DTGTK_SLIDER_BORDER_WIDTH * 3),
                vr.y + vr.height - (DTGTK_SLIDER_BORDER_WIDTH * 2));
  cairo_show_text(cr, sv);


  /* draw up/down arrows */
  dtgtk_cairo_paint_arrow(cr, width - DTGTK_SLIDER_ADJUST_BUTTON_WIDTH - DTGTK_SLIDER_BORDER_WIDTH,
                          DTGTK_SLIDER_BORDER_WIDTH * 2, DTGTK_SLIDER_ADJUST_BUTTON_WIDTH,
                          DTGTK_SLIDER_ADJUST_BUTTON_WIDTH - DT_PIXEL_APPLY_DPI(4), CPF_DIRECTION_UP);

  dtgtk_cairo_paint_arrow(cr, width - DTGTK_SLIDER_ADJUST_BUTTON_WIDTH - DTGTK_SLIDER_BORDER_WIDTH,
                          height - DTGTK_SLIDER_ADJUST_BUTTON_WIDTH + 4 - DTGTK_SLIDER_BORDER_WIDTH * 2,
...
GType dtgtk_slider_get_type()
{
  ...
  if(!dtgtk_slider_type)
  {
    static const GTypeInfo dtgtk_slider_info = {
      sizeof(GtkDarktableSliderClass), (GBaseInitFunc)NULL, (GBaseFinalizeFunc)NULL,
      (GClassInitFunc)_slider_class_init, NULL, /* class_finalize */
      NULL,                                     /* class_data */
      sizeof(GtkDarktableSlider), 0,            /* n_preallocs */
      (GInstanceInitFunc)_slider_init,
    };
    dtgtk_slider_type
        = g_type_register_static(GTK_TYPE_EVENT_BOX, "GtkDarktableSlider", &dtgtk_slider_info, 0);
  }
