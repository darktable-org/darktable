    ...
    int32_t pointery)
{
  ...
  if(dev->gui_module && dev->gui_module->request_color_pick != DT_REQUEST_COLORPICK_OFF && dev->gui_module->enabled)
  {
    ...
    float pwidth = (float)(dev->pipe->output_backbuf_width<<closeup) / darktable.gui->ppd;
    float pheight = (float)(dev->pipe->output_backbuf_height<<closeup) / darktable.gui->ppd;
    float hbar = ((float)self->width - pwidth) * .5f;
    float tbar = ((float)self->height - pheight) * .5f;
    cairo_save(cri);
