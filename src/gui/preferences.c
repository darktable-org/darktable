...
_dialog_bool_reset(GtkWidget *label, GdkEventButton *event, GtkWidget *widget)
{
  if(event->type == GDK_2BUTTON_PRESS)
  {
    ...
    gtk_toggle_button_set_active(GTK_TOGGLE_BUTTON(widget), default_value);
    return TRUE;
  }
  return FALSE;
}
...
GtkWidget *dt_preferences_dialog_bool(GtkWidget *grid, const char *setting)
{
  ...
  snprintf(tooltip, 1024, _("double click to reset to `%s'"),
           _(dt_confgen_get_bool(setting, DT_DEFAULT) ? "TRUE" : "FALSE"));
...
GtkWidget *dt_preferences_dialog_int(GtkWidget *grid, const char *setting)
{
  ...
  gtk_widget_add_events(labelev, GDK_BUTTON_PRESS_MASK);
  gtk_container_add(GTK_CONTAINER(labelev), label);
  snprintf(tooltip, 1024, _("double click to reset to `%d'"), dt_confgen_get_int(setting, DT_DEFAULT));
  g_object_set(labelev,  "tooltip-text", tooltip, (gchar *)0);
  gtk_event_box_set_visible_window(GTK_EVENT_BOX(labelev), FALSE);

  gint min = MAX(G_MININT, dt_confgen_get_int(setting, DT_MIN));
  gint max = MIN(G_MAXINT, dt_confgen_get_int(setting, DT_MAX));
  widget = gtk_spin_button_new_with_range(min, max, 1);
  gtk_spin_button_set_digits(GTK_SPIN_BUTTON(widget), 0);
  gtk_spin_button_set_value(GTK_SPIN_BUTTON(widget), dt_conf_get_int(setting));
...
static void
_dialog_enum_changed_callback(GtkWidget *widget, gpointer user_data)
{
  const char *name = gtk_widget_get_name(widget);
  set_widget_label_default(widget, name, GTK_WIDGET(user_data));
  GtkTreeIter iter;
  if(gtk_combo_box_get_active_iter(GTK_COMBO_BOX(widget), &iter))
  {
    ...
    dt_conf_set_string(name, s);
    g_free(s);
  }
}

static gboolean
_dialog_enum_reset(GtkWidget *label, GdkEventButton *event, GtkWidget *widget)
{
  gboolean found = FALSE;
  if(event->type == GDK_2BUTTON_PRESS)
  {
    const char *name = gtk_widget_get_name(widget);
    const char *default_value = dt_confgen_get(name, DT_DEFAULT);
...
GtkWidget *dt_preferences_dialog_enum(GtkWidget *grid, const char *setting)
{
  GtkWidget *widget, *label, *labelev, *box;
  char tooltip[1024];
  int line = _get_grid_nb_lines(GTK_GRID(grid));
  ...
  gtk_container_add(GTK_CONTAINER(labelev), label);
  // FIXME
  //  snprintf(tooltip, 1024, _("double click to reset to `%s'"), C_("preferences", "id"));
  snprintf(tooltip, 1024, _("double click to reset to `%s'"), _(dt_confgen_get(setting, DT_DEFAULT)));
  g_object_set(labelev, "tooltip-text", tooltip, (gchar *)0);
  gtk_event_box_set_visible_window(GTK_EVENT_BOX(labelev), FALSE);
  ...
  const char *values = dt_confgen_get(setting, DT_VALUES);
  gint i = 0;
  gint pos = -1;
  ...
  g_signal_connect(G_OBJECT(widget), "changed", G_CALLBACK(_dialog_enum_changed_callback), labdef);
  g_signal_connect(G_OBJECT(labelev), "button-press-event", G_CALLBACK(_dialog_enum_reset), (gpointer)widget);
