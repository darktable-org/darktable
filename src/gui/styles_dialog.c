                            ...
                            gpointer               data)
{
  ...
  if (toggle_item)
    gtk_list_store_set (GTK_LIST_STORE (model), &iter, DT_STYLE_ITEMS_COL_ENABLED, FALSE, -1);
  else
    gtk_list_store_set (GTK_LIST_STORE (model), &iter, DT_STYLE_ITEMS_COL_ENABLED, TRUE, -1);
