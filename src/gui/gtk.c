...
void dt_gui_close_splashscreen(void *splashscreen)
{
  ...
  splashscreen_t *s = (splashscreen_t *)splashscreen;
  s->running = FALSE;
  pthread_join(s->thread, NULL);
  free(s);
}

static gboolean splashscreen_check(gpointer user_data)
{
  splashscreen_t *splashscreen = (splashscreen_t *)user_data;
  ...
  if(splashscreen->image)
    gtk_widget_queue_draw(splashscreen->window);
  return TRUE;
}

#define SMOOTHSTEP(x) ((x) * (x) * (3 - 2 * (x)))

static gboolean splashscreen_draw(GtkWidget *widget, cairo_t *cr, gpointer user_data)
{
  splashscreen_t *splashscreen = (splashscreen_t *)user_data;

  if(splashscreen->has_alpha)
    cairo_set_source_rgba(cr, 0.3, 0.3, 0.3, 0.0);
  else
  ...
  if(splashscreen->image)
  {
    static const double pulse_amplitude = 0.02;
    static const double pulse_time_factor = 0.5;

    const double t = dt_get_wtime() - darktable.start_wtime;
    const double s = remainder(t * pulse_time_factor, 2.0);
    const double s_abs = fabs(s);
    const double smooth = SMOOTHSTEP(s_abs);

    const double scale = (1.0 - pulse_amplitude) + pulse_amplitude * smooth;
    const double translate = (1.0 - scale) * 0.5;

    cairo_translate(cr, translate * splashscreen->image_width, translate * splashscreen->image_height);
    cairo_scale(cr, scale, scale);

    cairo_set_source_surface(cr, splashscreen->image, 0, 0);
    cairo_rectangle(cr, 0, 0, splashscreen->image_width, splashscreen->image_height);
    cairo_fill(cr);
  }
...
static void *splashscreen_run(void *data)
{
  splashscreen_t *splashscreen = (splashscreen_t *)data;
  ...
  g_signal_connect(G_OBJECT(window), "draw", G_CALLBACK(splashscreen_draw), splashscreen);
  ...
  // using screen dpi/ppd here would be nice, but that information isn't yet available this early during init.
  splashscreen->image = dt_util_get_logo(150.0);
  splashscreen->image_width = cairo_image_surface_get_width(splashscreen->image);
  splashscreen->image_height = cairo_image_surface_get_height(splashscreen->image);

  gtk_widget_set_size_request(window, splashscreen->image_width, splashscreen->image_height);

  if(splashscreen->image && !splashscreen->has_alpha)
  {
    cairo_region_t *mask = gdk_cairo_region_create_from_surface(splashscreen->image);
  ...
  g_timeout_add(100, splashscreen_check, data);
  gtk_main();
  return NULL;
}
...
{
  splashscreen_t *splashscreen = (splashscreen_t *)malloc(sizeof(splashscreen_t));
  splashscreen->running = TRUE;

  dt_pthread_create(&(splashscreen->thread), splashscreen_run, splashscreen);
