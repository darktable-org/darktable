...
void _camera_storage_image_filename(const dt_camera_t *camera,const char *filename,CameraFile *preview,void *user_data) 
{
  ...
  if(pixbuf)
  {
    ...
    gtk_image_set_from_pixbuf(GTK_IMAGE(data->preview),thumb);
  }
...
void _camera_import_dialog_run(_camera_import_dialog_t *data) 
{
  ...
  // storeis now populated with thumbs and filenames lets present to user
  ...
  gtk_container_remove(GTK_CONTAINER( gtk_dialog_get_content_area (GTK_DIALOG (data->dialog) )),data->preview);
