...
//static void _gui_reinit(dt_lib_module_t *self);

static int _get_recentcollect_max_items();
                                          ...
                                          gpointer user_data)
{
  ...
  int n = -1;
  for(int k = 0; k < CLAMPS(dt_conf_get_int("plugins/lighttable/recentcollect/num_items"), 0, d->nb_items); k++)
  {
  ...
  if(n < 0)
  {
    const int num_items = CLAMPS(dt_conf_get_int("plugins/lighttable/recentcollect/num_items"), 0, d->nb_items);
    if(num_items < d->nb_items)
    {
      // new, unused entry
      n = num_items;
  ...
  GList *current = g_list_first(d->items);
  dt_lib_recentcollect_item_t *item;
  for(int k = 0 ; k < d->nb_items ; k++)
  {
    char str[2048] = { 0 };
    item = (dt_lib_recentcollect_item_t *)current->data;
    snprintf(confname, sizeof(confname), "plugins/lighttable/recentcollect/line%1d", k);
...
void _menuitem_preferences(GtkMenuItem *menuitem, dt_lib_module_t *self)
{
  ...
  if (gtk_dialog_run(GTK_DIALOG(dialog)) == GTK_RESPONSE_YES)
  {
    ...
    int old_nb_items = d->nb_items;
    int new_nb_items = _get_recentcollect_max_items();
    int delta = new_nb_items - old_nb_items;
    if (delta < 0)
    {
      // destroy old items
      current = g_list_nth(d->items, new_nb_items);
      while (current)
      {
        item = (dt_lib_recentcollect_item_t *)current->data;
    ...
    else if (delta > 0)
    {
      ...
      for(int k = old_nb_items ; k < new_nb_items ; k++)
      {
        ...
        item = (dt_lib_recentcollect_item_t *)malloc(sizeof(dt_lib_recentcollect_item_t));
...
void gui_reset(dt_lib_module_t *self)
{
  ...
  for(int k = 0; k < _get_recentcollect_max_items(); k++)
  {
    snprintf(confname, sizeof(confname), "plugins/lighttable/recentcollect/line%1d", k);
    dt_conf_set_string(confname, "");
    snprintf(confname, sizeof(confname), "plugins/lighttable/recentcollect/pos%1d", k);
...
void gui_init(dt_lib_module_t *self)
{
  ...
  d->nb_items = _get_recentcollect_max_items();
  d->items = NULL;
  self->data = (void *)d;
