...
#pragma omp parallel for default(none) \
  ...
  const size_t wf_8bit_stride = cairo_format_stride_for_width(CAIRO_FORMAT_A8,
...
#if defined(_OPENMP)
#pragma omp parallel for default(none) \
  dt_omp_firstprivate(partial_binned, bin_pad, wf_8bit, wf_8bit_stride, num_bins, num_tones, orient, lut, lutmax, scale, nthreads) \
    ...
    for(size_t bin = 0; bin < num_bins; bin++)
      for(size_t tone = 0; tone < num_tones; tone++)
      {
        ...
        if(orient == DT_LIB_HISTOGRAM_ORIENT_HORI)
          wf_8bit[(ch * num_tones + tone) * wf_8bit_stride + bin] = display;
        else
          wf_8bit[(ch * num_bins + bin) * wf_8bit_stride + tone] = display;
      }
...
static void _lib_histogram_draw_waveform_channel(dt_lib_histogram_t *d, cairo_t *cr, int ch, double alpha)
{
  ...
  {
    const size_t wf_8bit_stride = cairo_format_stride_for_width(CAIRO_FORMAT_A8, d->waveform_bins);
    surface
      = dt_cairo_image_surface_create_for_data(d->waveform_8bit + (2-ch) * d->waveform_tones * wf_8bit_stride,
                                               CAIRO_FORMAT_A8,
                                               d->waveform_bins, d->waveform_tones, wf_8bit_stride);
  }
  else
  {
    const size_t wf_8bit_stride = cairo_format_stride_for_width(CAIRO_FORMAT_A8, d->waveform_tones);
    surface
      = dt_cairo_image_surface_create_for_data(d->waveform_8bit + (2-ch) * d->waveform_bins * wf_8bit_stride,
                                               CAIRO_FORMAT_A8,
                                               d->waveform_tones, d->waveform_bins, wf_8bit_stride);
  }
...
static void _lib_histogram_draw_rgb_parade(dt_lib_histogram_t *d, cairo_t *cr, int width, int height)
{
  ...
  for(int ch = 2; ch >= 0; ch--)
  {
    ...
    if(d->scope_orient == DT_LIB_HISTOGRAM_ORIENT_HORI)
      cairo_translate(cr, d->waveform_bins/darktable.gui->ppd, 0.);
    else
      cairo_translate(cr, 0., d->waveform_bins/darktable.gui->ppd);
  }
  cairo_restore(cr);
}
...
static void _scope_orient_update(const dt_lib_histogram_t *d)
{
  switch(d->scope_orient)
  {
    ...
    case DT_LIB_HISTOGRAM_ORIENT_VERT:
      gtk_widget_set_tooltip_text(d->scope_view_button, _("set scale to horizontal"));
      dtgtk_button_set_paint(DTGTK_BUTTON(d->scope_view_button),
...
void gui_init(dt_lib_module_t *self)
{
  ...
  str = dt_conf_get_string("plugins/darkroom/histogram/orient");
