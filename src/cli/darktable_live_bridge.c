...
static gchar *build_lua_command(const gchar *command, gboolean have_exposure, double exposure)
{
  const char *lua_template =
    ...
    "  end\n"
    "\n"
    "  local function ensure_exposure_module_ready()\n"
    "    pcall(darktable.gui.action, 'iop/exposure', 'show', 'on', 1)\n"
    "    pcall(darktable.gui.action, 'iop/exposure', 'focus')\n"
    "  end\n"
    "\n"
    ...
    "    current_position, requested_render_sequence = wait_for_exposure_settle(image.id, action, exposure_ev_to_position(requested), sequence_before, history_before)\n"
