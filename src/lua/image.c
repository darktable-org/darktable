...
static int image_newindex(lua_State *L){
		  lua_image * my_image=dt_lua_checkimage(L,-3);
		  if(!my_image->image) {
					 my_image->image = dt_image_cache_write_get(darktable.image_cache,my_image->const_image);
		  }

		  switch(luaL_checkoption(L,-2,NULL,image_fields_name)) {
					 case EXIF_EXPOSURE:
								my_image->image->exif_exposure = luaL_checknumber(L,-1);
								return 0;
					 case EXIF_APERTURE:
								my_image->image->exif_aperture = luaL_checknumber(L,-1);
								return 0;
					 case EXIF_ISO:
								my_image->image->exif_iso = luaL_checknumber(L,-1);
								return 0;
					 case EXIF_FOCAL_LENGTH:
								my_image->image->exif_focal_length = luaL_checknumber(L,-1);
								return 0;
					 case EXIF_FOCUS_DISTANCE:
								my_image->image->exif_focus_distance = luaL_checknumber(L,-1);
								return 0;
					 case EXIF_CROP:
								my_image->image->exif_crop = luaL_checknumber(L,-1);
								return 0;
					 case EXIF_MAKER:
								{
										  size_t tgt_size;
