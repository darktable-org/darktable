...
lua_colorlabel *dt_lua_checkcolorlabel(lua_State * L,int index){
	lua_colorlabel* my_colorlabel= luaL_checkudata(L,index,LUA_COLORLABEL);
	return my_colorlabel;
}
...
void dt_lua_push_colorlabel(lua_State * L,int imgid) {
	...
	if(!lua_isnil(L,-1)) {
		...
		lua_pushinteger(L,imgid);
		lua_colorlabel * my_colorlabel = (lua_colorlabel*)lua_newuserdata(L,sizeof(lua_colorlabel));
...
static int colorlabel_index(lua_State *L){
	const lua_colorlabel * my_colorlabel=dt_lua_checkcolorlabel(L,-2);
	...
	if(value < 0 || value >= DT_COLORLABELS_LAST) {
		...
		lua_pushboolean(L,dt_colorlabels_check_label(my_colorlabel->imgid,value));
		return 1;
	}
}

static int colorlabel_newindex(lua_State *L){
	lua_colorlabel * my_colorlabel=dt_lua_checkcolorlabel(L,-3);
	...
	if(value < 0 || value >= DT_COLORLABELS_LAST) {
			...
			if(lua_toboolean(L,-1)) { // no testing of type so we can benefit from all types of values
				dt_colorlabels_set_label(my_colorlabel->imgid,value);
			} else {
				dt_colorlabels_remove_label(my_colorlabel->imgid,value);
			}
			return 0;
