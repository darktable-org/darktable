...
static gboolean key_softproof_callback(GtkAccelGroup *accel_group, GObject *acceleratable, guint keyval,
                                       GdkModifierType modifier, gpointer data)
{
  ...
  if(p->softproof_enabled)
  {
    ...
    if(filename) g_strlcpy(p->softproofprofile, filename, sizeof(p->softproofprofile));
  }
...
static gboolean key_gamutcheck_callback(GtkAccelGroup *accel_group, GObject *acceleratable, guint keyval,
                                        GdkModifierType modifier, gpointer data)
{
  dt_iop_module_t *self = (dt_iop_module_t *)data;
  dt_iop_colorout_gui_data_t *g = (dt_iop_colorout_gui_data_t *)self->gui_data;
...
static void display_intent_changed(GtkWidget *widget, gpointer user_data)
{
  dt_iop_module_t *self = (dt_iop_module_t *)user_data;
  if(self->dt->gui->reset) return;
  dt_iop_colorout_params_t *p = (dt_iop_colorout_params_t *)self->params;
...
static gchar *_get_profile_from_pos(GList *profiles, int pos)
{
  while(profiles)
  {
    ...
    if(pp->pos == pos) return pp->filename;
...
static gchar *_get_display_profile_from_pos(GList *profiles, int pos)
{
  ...
  {
    dt_iop_color_profile_t *pp = (dt_iop_color_profile_t *)profiles->data;
    if(pp->display_pos == pos) return pp->filename;
    profiles = g_list_next(profiles);
  }
  return NULL;
}
...
static void display_profile_changed(GtkWidget *widget, gpointer user_data)
{
  dt_iop_module_t *self = (dt_iop_module_t *)user_data;
  if(self->dt->gui->reset) return;
  dt_iop_colorout_params_t *p = (dt_iop_colorout_params_t *)self->params;
...
static cmsHPROFILE _create_profile(gchar *iccprofile)
{
  ...
  if(!profile) profile = dt_colorspaces_create_srgb_profile();
...
void init(dt_iop_module_t *module)
{
  ...
  dt_iop_colorout_params_t tmp
      = (dt_iop_colorout_params_t){ "sRGB", "X profile", DT_INTENT_PERCEPTUAL, DT_INTENT_PERCEPTUAL, 0,
                                    "sRGB", DT_INTENT_PERCEPTUAL };
...
{
  dt_accel_register_iop(self, FALSE, NC_("accel", "toggle softproofing"), GDK_KEY_s, GDK_CONTROL_MASK);

  dt_accel_register_iop(self, FALSE, NC_("accel", "toggle gamutcheck"), GDK_KEY_g, GDK_CONTROL_MASK);
}
