...
/* remarks:
   ...
   3) Reminders and possibly left to do:
      - halo supprssion at very strong gradients?
   ...
   4) Notes
      - If the demosaicer downscales effects are less visible so it seems safe to reduce the
        number of iterartions for performance.
*/
...
static inline int _get_iterations(const int level, const float scale)
{
  return floorf(CLIP(scale) * powf((float)level, 1.3f));
}
                                   ...
                                   const float boost)
{
  ...
  for(int row = 0; row < height; row++)
  {
    ...
    for(int col = 0; col < width; col++)
    {
      ...
      const float corr = MAX(0.0f, sc - 0.5f);
      float sigma = isigma + boost * powf(corr, 1.5f);

      // special care for the image borders
      if(col < 8)               sigma *= (float)col * 0.125f;
      else if(row < 8)          sigma *= (float)row * 0.125f;
      else if(col > width - 8)  sigma *= (float)(width - col) * 0.125f;
      else if(row > height - 8) sigma *= (float)(height - row) * 0.125f;

      const int kern = sigma / CAPTURE_GAUSS_FRACTION;
      table[row * width + col] = CLAMP(kern, 0, UCHAR_MAX);
    }
  }
  return table;
}
...
DT_OMP_DECLARE_SIMD(aligned(in, out, kernels:64))
    ...
    for(int col = 0; col < width; col++)
    {
      ...
      if(blend[i] > CAPTURE_BLEND_EPS)
      {
        ...
        if(col >= 4 && row >= 4 && col < width - 4 && row < height - 4)
        {
          const float *d = in + i;
          val =
              kern[10+4] * (d[-w4-h2]  + d[-w4+h2]  + d[-w2-h4]  + d[-w2+h4] + d[w2-h4] + d[w2+h4] + d[w4-h2] + d[w4+h2]) +
...
DT_OMP_DECLARE_SIMD(aligned(in, out, divbuff, kernels :64))
static inline void _blur_div(const float *const in,
                             float *out,
                             const float *const divbuff,
                             const float *blend,
                             const float *const kernels,
                             const unsigned char *const table,
                             const int width,
  ...
  for(int row = 0; row < height; row++)
  {
    for(int col = 0; col < width; col++)
    {
      ...
      if(blend[i] > CAPTURE_BLEND_EPS)
      {
        ...
        out[i] = divbuff[i] / MAX(val, 0.00001f);
      }
      else
        out[i] = in[i];
    }
...
static void _prepare_blend(const float *cfa,
                           ...
                           const int height)
{
  dt_iop_image_fill(mask, 1.0f, width, height, 1);
  const int w2 = 2 * width;
  ...
  for(size_t row = 0; row < height; row++)
  {
    ...
    {
      const size_t k = row * width + col;
      // Photometric/digital ITU BT.709 rgb -> Y
      const float Y = 0.2126f*rgb[k*4] + 0.7152f*rgb[k*4+1] + 0.0722f*rgb[k*4+2];
      Yold[k] = Y;
      luminance[k] = Y;
      if(row > 1 && col > 1 && row < height-2 && col < width -2)
      {
        const int color = (filters == 9u) ? FCxtrans(row, col, roi, xtrans) : FC(row, col, filters);
        if(cfa[k] > 0.9f * whites[color] || Y < 0.005f)
        {
    ...
    for(int icol = 0; icol < width; icol++)
    {
      ...
      for(int y = row-1; y < row+2; y++)
      {
        for(int x = col-2; x < col+3; x++)
          av += luminance[(size_t)y*width + x];
      }
      for(int x = col-1; x < col+2; x++)
      {
        av += luminance[(size_t)(row-2)*width + x];
        av += luminance[(size_t)(row+2)*width + x];
      }
      av /= 21.0f;
      ...
      {
        for(int x = col-2; x < col+3; x++)
          sv += sqrf(luminance[(size_t)y*width + x] - av);
      }
      for(int x = col-1; x < col+2; x++)
      {
        sv += sqrf(luminance[(size_t)(row-2)*width + x] - av);
        sv += sqrf(luminance[(size_t)(row+2)*width + x] - av);
      }
      sv = (float)VARLUT_SIZE * 10.0f * (sqrtf(sv / 21.f) - 0.08f * threshold);
      const int idx = CLAMP((int)sv + VARLUT_SIZE, 0, VARLUT_SIZE-1);
      blend[k] *= CLIP(lut[idx]);
    }
  }
}
                      ...
                      const gboolean showmask)
{
  ...
  if(!luminance || !tmp2 || !tmp1 || !blendmask || !gd->var_lut)
    goto finalize;

  /*  tmp2 will hold the temporary clipmask later modified by luminance-based variance
      luminance to be used later
      tmp1 holds Y data
  */
  _prepare_blend(in, out, filters, xtrans, roi, tmp2, luminance, tmp1, icoeffs, d->cs_thrs, width, height);
  _modify_blend(tmp2, luminance, gd->var_lut, d->cs_thrs, width, height);
  ...
  {
    const float luminance_new = interpolatef(blendmask[k], tmp1[k], luminance[k]);
    const float factor = luminance_new / MAX(luminance[k], 0.00001f);
    for_three_channels(c) out[k*4+c] = out[k*4+c] * factor;
  }
                        ...
                        const gboolean showmask)
{
  ...
  const int width = roi->width;
  const int height = roi->height;
  const int bsize = sizeof(float) * width * height;
  ...
  const dt_iop_buffer_dsc_t *dsc = &piece->pipe->dsc;
  const gboolean wbon = dsc->temperature.enabled;
  dt_aligned_pixel_t icoeffs = { wbon ? dsc->temperature.coeffs[0] : 1.0f,
                                 wbon ? dsc->temperature.coeffs[1] : 1.0f,
                                 wbon ? dsc->temperature.coeffs[2] : 1.0f,
                                 0.0f };
  const gboolean autoradius = (pipe->type & DT_DEV_PIXELPIPE_FULL)
  ...
  cl_mem lut = gd->var_lut ? dt_opencl_copy_host_to_device_constant(devid, VARLUT_SIZE * sizeof(float), gd->var_lut) : NULL;
  cl_mem xtrans = dt_opencl_copy_host_to_device_constant(devid, sizeof(piece->pipe->dsc.xtrans), piece->pipe->dsc.xtrans);
  ...
  if(!blendmask || !luminance || !tmp2 || !tmp1 || !xtrans || !whites || !dev_rgb || !lut) goto finish;
  ...
  err = dt_opencl_enqueue_kernel_2d_args(devid, gd->prepare_blend, width, height,
          CLARG(dev_in), CLARG(dev_out), CLARG(filters), CLARG(xtrans), CLARG(tmp2), CLARG(luminance), CLARG(tmp1),
          CLARG(whites), CLARG(width), CLARG(height));
  if(err != CL_SUCCESS) goto finish;

  err = dt_opencl_enqueue_kernel_2d_args(devid, gd->modify_blend, width, height,
          CLARG(tmp2), CLARG(luminance), CLARG(lut), CLARG(d->cs_thrs), CLARG(width), CLARG(height));
  if(err != CL_SUCCESS) goto finish;
  ...
  if(showmask)
  {
    err = dt_opencl_enqueue_kernel_2d_args(devid, gd->show_blend_mask, width, height,
          CLARG(dev_rgb), CLARG(dev_out), CLARG(blendmask), CLARG(width), CLARG(height));
    goto finish;
  }

  unsigned char *f_gauss_idx = _cs_precalc_gauss_idx(self, roi, radius, d->cs_boost);
  if(f_gauss_idx)
  {
  ...
  dt_opencl_release_mem_object(lut);
