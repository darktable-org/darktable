                         ...
                         const int margin)
{
  ...
  for(int j = 0; j < height; j++)
    for(int i = 0; i < width; i++)
    {
      ...
      for(int c = 0; c < 3; c++)
      {
        const float oval = (c != f && sum[c + 4] > 0.0f) ? sum[c] / sum[c + 4] : in[(size_t)j * width + i];
        out[4 * ((size_t)j * width + i) + c] = fmaxf(DEMOSAIC_OUTMIN, oval);
      }
    }
  ...
  for(int j = 3; j < height - 3; j++)
  {
    ...
    for(int i = 3; i < width - 3; i++)
    {
      ...
      for_each_channel(k,aligned(buf,color:16)
        dt_omp_nontemporal(buf)) buf[k] = fmaxf(DEMOSAIC_OUTMIN, color[k]);
