...
static void rt_show_hide_controls(const dt_iop_module_t *self)
{
  ...
  const dt_masks_form_t *form =
    dt_masks_get_from_id(darktable.develop, rt_get_selected_shape_id());
...
static void rt_shape_selection_changed(dt_iop_module_t *self)
{
  ...
  if(index >= 0)
  {
    ...
    if(p->algorithm != p->rt_forms[index].algorithm)
    {
      ...
      gtk_toggle_button_set_active(GTK_TOGGLE_BUTTON(g->bt_heal),
                                   (p->algorithm == DT_IOP_RETOUCH_HEAL));
      gtk_toggle_button_set_active(GTK_TOGGLE_BUTTON(g->bt_blur),
  ...
  if(bd->masks_shown == DT_MASKS_EDIT_OFF || count == 0)
  {
    ...
    if(g)
      gtk_toggle_button_set_active
        (GTK_TOGGLE_BUTTON(g->bt_edit_masks),
        ...
        forms = g_list_next(forms))
    {
      ...
      if(grpt)
      {
        ...
        else
        {
          ...
          const dt_masks_form_t *parent_form =
                                        ...
                                        const dt_iop_roi_t *roi_out)
{
  ...
  if(ft >= roi_out->y + roi_out->height
     || ft + fh <= roi_out->y
     || fl >= roi_out->x + roi_out->width
     || fl + fw <= roi_out->x)
    return FALSE;
...
static void rt_clamp_minmax(float levels_old[3], float levels_new[3])
{
  ...
  if((levels_old[0] != levels_new[0]
      || levels_old[2] != levels_new[2])
     && levels_old[1] == levels_new[1])
  {
    ...
    if(levels_old[2] != levels_old[0])
    {
      ...
      const float percentage =
        (levels_old[1] - levels_old[0]) / (levels_old[2] - levels_old[0]);
  ...
  // check the range
  if(levels_new[2] < levels_new[0] + 0.05f * 2.f)
    levels_new[2] = levels_new[0] + 0.05f * 2.f;
  if(levels_new[1] < levels_new[0] + 0.05f)
    levels_new[1] = levels_new[0] + 0.05f;
  if(levels_new[1] > levels_new[2] - 0.05f)
    levels_new[1] = levels_new[2] - 0.05f;

  {
    ...
    const float percentage =
      (levels_new[1] - levels_new[0]) / (levels_new[2] - levels_new[0]);
    levels_new[1] = left + (right - left) * percentage;
    levels_new[0] = left;
    levels_new[2] = right;
  }
...
static gboolean rt_shape_is_being_added(dt_iop_module_t *self,
                                        const dt_masks_type_t shape_type)
{
  gboolean being_added = FALSE;

  if(self->dev->form_gui && self->dev->form_visible
     && ((self->dev->form_gui->creation
          && self->dev->form_gui->creation_module == self)
         || (self->dev->form_gui->creation_continuous
             && self->dev->form_gui->creation_continuous_module == self)))
  {
    if(self->dev->form_visible->type & DT_MASKS_GROUP)
    {
      ...
      if(forms)
      {
        ...
        {
          const dt_masks_form_t *form =
            dt_masks_get_from_id(darktable.develop, grpt->formid);
          if(form)
            being_added = (form->type & shape_type);
        }
      }
    }
    else
...
static gboolean rt_add_shape(GtkWidget *widget,
                             const int creation_continuous,
                             dt_iop_module_t *self)
{
                                 ...
                                 dt_iop_module_t *self)
{
  ...
  if(p->num_scales == num_scales)
    return;

  p->num_scales = num_scales;

  if(p->num_scales < p->merge_from_scale)
    p->merge_from_scale = p->num_scales;
...
static void rt_curr_scale_update(const int _curr_scale, dt_iop_module_t *self)
{
  ...
  if(g->displayed_wavelet_scale == 0
     && p->preview_levels[0] == RETOUCH_PREVIEW_LVL_MIN
     && p->preview_levels[1] == 0.f
     && p->preview_levels[2] == RETOUCH_PREVIEW_LVL_MAX
     && g->preview_auto_levels == 0
     && p->curr_scale > 0 && p->curr_scale <= p->num_scales)
  {
                                       ...
                                       dt_iop_module_t *self)
{
  ...
  {
    g->upper_margin = TRUE;
    const float middle = box_w * (0.5f + (float)p->merge_from_scale);
    g->upper_cursor = (g->wdbar_mouse_x >= (middle - inset))
  ...
  {
    g->lower_margin = TRUE;
    const float middle = box_w * (0.5f + (float)p->num_scales);
    g->lower_cursor = (g->wdbar_mouse_x >= (middle - inset))
      && (g->wdbar_mouse_x <= (middle + inset));
    if(!(g->is_dragging))
      g->curr_scale = -1;
  }
                              ...
                              const dt_iop_module_t *self)
{
  ...
  float middle;
  const int first_scale_visible = (g->first_scale_visible > 0)
  ...
  for(int i = 0; i < RETOUCH_NO_SCALES; i++)
  {
    ...
    if(rt_scale_has_shapes(p, i))
    {
      ...
      cairo_rectangle(cr, box_w * i + inset + lw / 2.0f,
                      allocation.height - sh, box_w - lw, 2.0f * lw);
      cairo_fill(cr);
    }
  ...
  if(p->curr_scale >= p->merge_from_scale
     && p->curr_scale <= p->num_scales
  ...
  if(g->curr_scale >= 0)
  {
    ...
    gdk_cairo_set_source_rgba(cr, &color);
    cairo_rectangle(cr, box_w * g->curr_scale + inset + lw,
                    sh + lw, box_w - 2.0f * lw, box_h - 2.0f * lw);
    cairo_stroke(cr);
  }
  ...
  if(g->lower_cursor || g->is_dragging == DT_IOP_RETOUCH_WDBAR_DRAG_BOTTOM)
  {
    ...
    dtgtk_cairo_paint_solid_triangle(cr, middle, box_h + 5.0f * lw,
                                     mk, mk, CPF_DIRECTION_UP, NULL);
  }
  else
  {
    cairo_set_source_rgb(cr, 0.54, 0.54, 0.54);
    dtgtk_cairo_paint_triangle(cr, middle, box_h + 5.0f * lw, mk, mk,
  ...
  {
    cairo_set_source_rgb(cr, 0.67, 0.67, 0.67);
    dtgtk_cairo_paint_solid_triangle(cr, middle, 3.0f * lw, mk, mk,
                                     CPF_DIRECTION_DOWN, NULL);
  }
  else
  {
    cairo_set_source_rgb(cr, 0.54, 0.54, 0.54);
    dtgtk_cairo_paint_triangle(cr, middle, 3.0f * lw, mk, mk,
                               CPF_DIRECTION_DOWN, NULL);
  }
                                       ...
                                       const int dir)
{
  ...
  switch(dir)
  {
    case GRADIENT_SLIDER_SET:
      outval = (inval - RETOUCH_PREVIEW_LVL_MIN)
        / (RETOUCH_PREVIEW_LVL_MAX - RETOUCH_PREVIEW_LVL_MIN);
      break;
    case GRADIENT_SLIDER_GET:
      outval = (RETOUCH_PREVIEW_LVL_MAX - RETOUCH_PREVIEW_LVL_MIN)
        * inval + RETOUCH_PREVIEW_LVL_MIN;
      break;
    default:
      outval = inval;
  }
...
static void rt_gslider_changed(GtkDarktableGradientSlider *gslider,
                               dt_iop_module_t *self)
{
  dt_iop_retouch_params_t *p = self->params;
                                            ...
                                            dt_iop_module_t *self)
{
  ...
  gtk_toggle_button_set_active(GTK_TOGGLE_BUTTON(g->bt_copy_scale),
                               g->copied_scale >= 0);
  gtk_toggle_button_set_active(GTK_TOGGLE_BUTTON(g->bt_paste_scale),
                               g->copied_scale >= 0);
                                      ...
                                      dt_iop_module_t *self)
{
  ...
  gtk_toggle_button_set_active(GTK_TOGGLE_BUTTON(g->bt_circle),
                               rt_shape_is_being_added(self, DT_MASKS_CIRCLE));
  gtk_toggle_button_set_active(GTK_TOGGLE_BUTTON(g->bt_path),
                               rt_shape_is_being_added(self, DT_MASKS_PATH));
  gtk_toggle_button_set_active(GTK_TOGGLE_BUTTON(g->bt_ellipse),
                                             ...
                                             dt_iop_module_t *self)
{
  ...
  else if(darktable.develop->form_gui->creation
          && (darktable.develop->form_gui->creation_module == self))
  {
    dt_iop_request_focus(self);
...
void init_global(dt_iop_module_so_t *self)
{
  ...
  gd->kernel_retouch_copy_alpha =
    dt_opencl_create_kernel(program, "retouch_copy_alpha");
  gd->kernel_retouch_copy_buffer_to_buffer =
    dt_opencl_create_kernel(program, "retouch_copy_buffer_to_buffer");
  gd->kernel_retouch_copy_buffer_to_image =
    dt_opencl_create_kernel(program, "retouch_copy_buffer_to_image");
  gd->kernel_retouch_fill =
    dt_opencl_create_kernel(program, "retouch_fill");
  ...
  gd->kernel_retouch_image_rgb2lab =
    dt_opencl_create_kernel(program, "retouch_image_rgb2lab");
  gd->kernel_retouch_image_lab2rgb =
                     ...
                     dt_develop_tiling_t *tiling)
{
  ...
  const float require_cl = 1.0f  // in_retouch
     + ((p->num_scales > 0) ? 4.0f : 2.0f); // dwt_wavelet_decompose_cl
                                            // requires 4 buffers,
                                            // otherwise 2.0f is
                                            // enough

  // FIXME the above are worst case values, we might iterate through
  // the dt_iop_retouch_form_data_t to get the largest bounding box
...
void gui_init(dt_iop_module_t *self)
{
  ...
  const dt_iop_retouch_params_t *const p = self->default_params;
  ...
  gtk_grid_attach(GTK_GRID(grid_wd_labels),
                  GTK_WIDGET(g->lbl_curr_scale), 1, 1, 1, 1);

  gtk_grid_attach(GTK_GRID(grid_wd_labels),
                  dt_ui_label_new(_("merge from:")), 0, 2, 1, 1);
  g->lbl_merge_from_scale = GTK_LABEL(dt_ui_label_new(NULL));
  gtk_label_set_width_chars(g->lbl_merge_from_scale, 2);
  gtk_grid_attach(GTK_GRID(grid_wd_labels),
                  GTK_WIDGET(g->lbl_merge_from_scale), 1, 2, 1, 1);
  ...
  gtk_widget_set_tooltip_text
    (g->cmb_fill_mode, _("erase the detail or fills with chosen color"));
    ...
    for(const GList *forms = grp->points; forms; forms = g_list_next(forms))
    {
      ...
      if(grpt)
      {
        ...
        if(form)
        {
          ...
          if(ft >= roi_in->y + roi_in->height
             || ft + fh <= roi_in->y
             || fl >= roi_in->x + roi_in->width
             || fl + fw <= roi_in->x)
          {
            continue;
          }
          ...
          if(p->rt_forms[index].algorithm == DT_IOP_RETOUCH_BLUR)
          {
            ...
            {
              const int overlap =
                ceilf(4 * (p->rt_forms[index].blur_radius * roi_in->scale / piece->iscale));
              if(roiy > ft)
                roiy = MAX(roiy - overlap, ft);
              if(roix > fl)
                roix = MAX(roix - overlap, fl);
              if(roir < fl + fw)
                roir = MAX(roir + overlap, fl + fw);
              if(roib < ft + fh)
                roib = MAX(roib + overlap, ft + fh);
            }
          }
    ...
    for(const GList *forms = grp->points; forms; forms = g_list_next(forms))
    {
      ...
      if(grpt)
      {
        ...
        if(form)
        {
          ...
          const int intersects = !(ft_dest + fh < ft_src
                                   || ft_src + fh_src < ft_dest
                                   || fl_dest + fw < fl_src
  ...
  if(grp && (grp->type & DT_MASKS_GROUP))
  {
    for(const GList *forms = grp->points; forms; forms = g_list_next(forms))
    {
      ...
      if(grpt)
      {
        ...
        if(!dt_masks_get_source_area(self, piece, form,
                                     &fw_src, &fh_src, &fl_src, &ft_src))
        {
          continue;
        }
        ...
        const int intersects
            = !(roib < ft_src
                ...
                || fl_src + fw_src < roix);
        if(intersects)
          rt_extend_roi_in_from_source_clones(self, piece, roi_in,
                                              formid, fl_src, ft_src, fw_src, fh_src,
                                              &roir, &roib, &roix, &roiy);
      }
    }
  }
                   ...
                   dt_iop_roi_t *roi_in)
{
  *roi_in = *roi_out;
  ...
  const float scwidth = piece->buf_in.width * roi_in->scale;
                             ...
                             const float levels[3])
{
  ...
  if(left == RETOUCH_PREVIEW_LVL_MIN && middle == 0.f
     && right == RETOUCH_PREVIEW_LVL_MAX) return;
...
{
  const int x_from = MAX(MAX((roi_1->x + 1 - padding),
                             roi_2->x),
                         (roi_2->x + dx));
  const int x_to
      = MIN(MIN((roi_1->x + roi_1->width + 1 + padding),
                roi_2->x + roi_2->width),
            (roi_2->x + roi_2->width + dx));

  const int y_from = MAX(MAX((roi_1->y + 1 - padding),
                             roi_2->y),
                         (roi_2->y + dy));
  const int y_to = MIN(MIN((roi_1->y + roi_1->height + 1 + padding),
                          ...
                          dt_dev_pixelpipe_iop_t *piece)
{
  ...
  if(blur_type == DT_IOP_RETOUCH_BLUR_GAUSSIAN && fabsf(blur_radius) > 0.1f)
  {
    ...
    dt_gaussian_t *g = dt_gaussian_init(roi_mask_scaled->width,
                                        roi_mask_scaled->height,
                                        4, Labmax, Labmin, sigma,
  ...
  else if(blur_type == DT_IOP_RETOUCH_BLUR_BILATERAL && fabsf(blur_radius) > 0.1f)
  {
    ...
    dt_bilateral_t *b = dt_bilateral_init(roi_mask_scaled->width,
    ...
    if(b)
    {
      ...
      if(work_profile)
        dt_ioppr_transform_image_colorspace(self, img_dest, img_dest,
                          ...
                          const int max_iter)
{
  ...
  dt_heal(img_src, img_dest, mask_scaled,
          roi_mask_scaled->width, roi_mask_scaled->height, 4, max_iter);
...
static void rt_process_forms(float *layer, dwt_params_t *const wt_p, const int scale1)
{
  ...
  // when the requested scales is grather than max scales the residual
  // image index will be different from the one defined by the user,
  // so we need to adjust it here, otherwise we will be using the
  // shapes from a scale on the residual image
  ...
  if(!usr_d->suppress_mask)
  {
    ...
    if(grp && (grp->type & DT_MASKS_GROUP))
    {
      for(const GList *forms = grp->points; forms; forms = g_list_next(forms))
      {
        ...
        dt_masks_get_mask(self, piece, form, &mask,
        ...
        rt_build_scaled_mask(mask, &roi_mask,
                             &mask_scaled, &roi_mask_scaled, roi_layer, dx, dy, algo);
               ...
               && (roi_mask_scaled.height > 2)))
        {
          ...
          {
            _retouch_blur(self, layer, roi_layer, mask_scaled,
                          &roi_mask_scaled, form_opacity,
                          p->rt_forms[index].blur_type,
                          p->rt_forms[index].blur_radius, piece);
          }
          else if(algo == DT_IOP_RETOUCH_FILL)
          {
            ...
            {
              fill_color[0] =
                p->rt_forms[index].fill_color[0] + p->rt_forms[index].fill_brightness;
              fill_color[1] =
                p->rt_forms[index].fill_color[1] + p->rt_forms[index].fill_brightness;
              fill_color[2] =
                p->rt_forms[index].fill_color[2] + p->rt_forms[index].fill_brightness;
            }
            fill_color[3] = 0.0f;
             ...
             const dt_iop_roi_t *const roi_out)
{
     ...
     && dt_iop_has_focus(self) && (piece->pipe == self->dev->full.pipe))
  {
    ...
    piece->pipe->mask_display =
      g->mask_display ? DT_DEV_PIXELPIPE_DISPLAY_MASK : DT_DEV_PIXELPIPE_DISPLAY_PASSTHRU;
...
static cl_int rt_build_scaled_mask_cl(const int devid,
                                      float *const mask,
                                      dt_iop_roi_t *const roi_mask,
                                      float **mask_scaled,
...
static cl_int rt_copy_image_masked_cl(const int devid,
                                      cl_mem dev_src,
                                      cl_mem dev_dest,
                                      dt_iop_roi_t *const roi_dest,
...
static cl_int rt_copy_mask_to_alpha_cl(const int devid,
                                       cl_mem dev_layer,
...
static cl_int _retouch_clone_cl(const int devid,
                                cl_mem dev_layer,
                                dt_iop_roi_t *const roi_layer,
                                cl_mem dev_mask_scaled,
                                dt_iop_roi_t *const roi_mask_scaled,
...
static cl_int _retouch_fill_cl(const int devid,
                               cl_mem dev_layer,
                               dt_iop_roi_t *const roi_layer,
                               cl_mem dev_mask_scaled,
                               dt_iop_roi_t *const roi_mask_scaled,
...
static cl_int _retouch_blur_cl(const int devid,
                               cl_mem dev_layer,
                               dt_iop_roi_t *const roi_layer,
                               cl_mem dev_mask_scaled,
                               dt_iop_roi_t *const roi_mask_scaled,
                               ...
                               const int max_iter)
{
  ...
  if(hp)
  {
    err = dt_heal_cl(hp, dev_src, dev_dest, mask_scaled,
                     roi_mask_scaled->width, roi_mask_scaled->height, max_iter);
    dt_heal_free_cl(hp);
...
static cl_int rt_process_forms_cl(cl_mem dev_layer,
                                  dwt_params_cl_t *const wt_p,
                                  const int scale1)
{
  ...
  if(!usr_d->suppress_mask)
  {
    ...
    if(grp && (grp->type & DT_MASKS_GROUP))
    {
      for(const GList *forms = grp->points;
          ...
          forms = g_list_next(forms))
      {
        ...
        dt_masks_get_mask(self, piece, form, &mask,
                          &roi_mask.width, &roi_mask.height,
        ...
        err = rt_build_scaled_mask_cl(devid, mask,
                                      &roi_mask,
                                      &mask_scaled,
                                      &dev_mask_scaled,
                                      &roi_mask_scaled,
           ...
           && ((roi_mask_scaled.width > 2) && (roi_mask_scaled.height > 2)))
        {
          ...
          {
            err = _retouch_blur_cl(devid, dev_layer, roi_layer,
          ...
          else if(algo == DT_IOP_RETOUCH_FILL)
          {
            ...
            err = _retouch_fill_cl(devid, dev_layer, roi_layer,
               ...
               const dt_iop_roi_t *const roi_out)
{
  ...
  if(dwt_p->return_layer > 0
     && dwt_p->return_layer < dwt_p->scales + 1)
  {
    err = rt_adjust_levels_cl(self, piece, devid, in_retouch,
                              roi_rt->width, roi_rt->height, levels);
    if(err != CL_SUCCESS) goto cleanup;
  }
