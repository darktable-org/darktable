               ...
               const dt_iop_roi_t *const roi_in, const dt_iop_roi_t *const roi_out)
{
  ...
  if(d->operator== OPERATOR_DRAGO)
  {
    ...
    if(isnan(tmp_lwmax))
    {
      ...
      maximum = dt_malloc_aligned(16, reducesize * sizeof(float));
...
{
  piece->data = dt_calloc(1, sizeof(dt_iop_global_tonemap_data_t));
...
{
  module->params = dt_calloc(1, sizeof(dt_iop_global_tonemap_params_t));
  module->default_params = dt_calloc(1, sizeof(dt_iop_global_tonemap_params_t));
