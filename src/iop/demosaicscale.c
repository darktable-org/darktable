...
const char *name()
{
  return C_("modulename", "crop and scale input");
}

int flags()
{
  return IOP_FLAGS_ALLOW_TILING | IOP_FLAGS_HIDDEN | IOP_FLAGS_FENCE
                   ...
                   dt_dev_pixelpipe_iop_t *piece)
{
  ...
  /** FIXME currently we don't support linear sraw images as that requires
      some work in highlichts module.
      Yet - that would be very good as
      - the chroma correction factors would always be calculated from full image data
      - segmentation could be implemented that way.
  */
  const gboolean supported = dt_image_is_mono_sraw(img) || dt_image_is_raw(img);
  const gboolean scale = supported && dt_pipe_is_canvas(pipe);
  if(!scale)
    piece->enabled = FALSE;

  /** avoid on full pipe if HQ processing is enabled as that scales anyway.
      FIXME: probably works also for preview2 pipe and while running
      any pipe via dt_dev_image() like slideshow, snapshots and duplicates.
  */
  if(dt_pipe_is_full(pipe) && darktable.develop->late_scaling.enabled)
    piece->enabled = FALSE;
}
