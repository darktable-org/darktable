...
#pragma omp parallel for default(none) shared(in, out) schedule(static)
  ...
  float *scanline = dt_calloc(size, sizeof(float));
...
{
  piece->data = dt_calloc(1, sizeof(dt_iop_highpass_data_t));
...
{
  module->params = dt_calloc(1, sizeof(dt_iop_highpass_params_t));
  module->default_params = dt_calloc(1, sizeof(dt_iop_highpass_params_t));
