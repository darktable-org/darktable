...
void diffuse_x(matrix<float> &developer_concentration, int convlength, int convrad, int pad, int paddedwidth,
               int order, float swell_factor);

void diffuse(matrix<float> &developer_concentration, float sigma_const, float pixels_per_millimeter,
             float timestep)
{
  ...
  diffuse_x(developer_concentration, convlength, convrad, pad, paddedwidth, order, swell_factor);
  ...
  diffuse_x(transposed_developer_concentration, convlength, convrad, pad, paddedheight, order, swell_factor);
...
#ifdef _OPENMP
#pragma omp parallel shared(developer_concentration, convlength, convrad, order, length, width, paddedwidth, \
                            pad, swell_factor) firstprivate(row, col, pass, htemp, running_sum, hpadded)
#endif
  {
#ifdef _OPENMP
#pragma omp for schedule(dynamic) nowait
    ...
    for(row = 0; row < length; row++)
    {
      ...
      {
        hpadded[col + pad + width] = developer_concentration(row, width - 2 - col);
      }
      for(pass = 0; pass < order; pass++)
      {
        ...
        for(col = (pass + 1) * convrad; col < paddedwidth - 1 - (pass + 1) * convrad; col++)
        {
