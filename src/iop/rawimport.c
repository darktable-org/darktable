...
static void
ca_autocorrect_callback (GtkToggleButton *toggle, gpointer user_data)
{
  dt_iop_module_t *self = (dt_iop_module_t *)user_data;
  dt_iop_rawimport_params_t *p = (dt_iop_rawimport_params_t *)self->params;
  int active = gtk_toggle_button_get_active(toggle);
...
void gui_init(struct dt_iop_module_t *self)
{
  ...
  gtk_table_attach(GTK_TABLE(self->widget), GTK_WIDGET(g->greeneq), 4, 7, 3, 4, GTK_EXPAND|GTK_FILL, 0, 0, 0);
  ...
  gtk_object_set(GTK_OBJECT(g->amaze_ca_correct), "tooltip-text", _("automatic chromatic aberration correction from AMaZE"), (char *)NULL);
  gtk_table_attach(GTK_TABLE(self->widget), GTK_WIDGET(g->amaze_ca_correct), 4, 7, 5, 6, GTK_EXPAND|GTK_FILL, 0, 0, 0);

  //GtkComboBox *noiserd_fbdd;
  GtkWidget *fbdd_noise = gtk_label_new(_("denoising"));
  gtk_misc_set_alignment(GTK_MISC(fbdd_noise), 0.0, 0.5);
  ...
  gtk_object_set(GTK_OBJECT(g->noiserd_fbdd), "tooltip-text", _("turns on pre-demosaicing luma (and optionally chroma) noise reduction"), (char *)NULL);
  gtk_table_attach(GTK_TABLE(self->widget), fbdd_noise, 0, 3, 6, 7, GTK_EXPAND|GTK_FILL, 0, 0, 0);
  gtk_table_attach(GTK_TABLE(self->widget), GTK_WIDGET(g->noiserd_fbdd), 4, 7, 6, 7, GTK_EXPAND|GTK_FILL, 0, 0, 0);
  ...
  gtk_table_attach(GTK_TABLE(self->widget), GTK_WIDGET(reset), 0, 3, 8, 9, GTK_EXPAND|GTK_FILL, 0, 0, 0);
  gtk_table_attach(GTK_TABLE(self->widget), GTK_WIDGET(reload), 4, 7, 8, 9, GTK_EXPAND|GTK_FILL, 0, 0, 0);
  ...
  g_signal_connect (G_OBJECT (g->amaze_ca_correct), "toggled", G_CALLBACK (ca_autocorrect_callback), self);
}
