...
void modify_roi_in(struct dt_iop_module_t *self, struct dt_dev_pixelpipe_iop_t *piece, const dt_iop_roi_t *roi_out, dt_iop_roi_t *roi_in)
{
  ...
  *roi_in = *roi_out;
  ...
  roi_in->x = CLAMP(roi_in->x, 0, floorf(w));
  roi_in->y = CLAMP(roi_in->y, 0, floorf(h));
  roi_in->width = CLAMP(roi_in->width, 1, ceilf(w) - roi_in->x);
  roi_in->height = CLAMP(roi_in->height, 1, ceilf(h) - roi_in->y);
}
