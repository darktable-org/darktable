               ...
               const dt_iop_roi_t *const roi_in, const dt_iop_roi_t *const roi_out)
{
  ...
  err = dt_opencl_enqueue_kernel_2d_args(devid, kernel, width, height,
    CLARG(dev_in), CLARG(dev_out), CLARG(width), CLARG(height), CLARG(dev_m), CLARG(dev_l), CLARG(dev_r), CLARG(dev_g), CLARG(dev_b), CLARG(blue_mapping), CLARG(dev_coeffs));
  if(err != CL_SUCCESS) goto error;
...
error:
  ...
  dt_opencl_release_mem_object(dev_b);
  dt_opencl_release_mem_object(dev_coeffs);
  dt_print(DT_DEBUG_OPENCL, "[opencl_colorin] couldn't enqueue kernel! %s\n", cl_errstr(err));
  return FALSE;
}
#endif
