...
{
  GtkVBox   *vbox1,  *vbox2;
  GtkLabel  *label1,*label2,*label3,*label4;			                	// scale, strength, uniformity, b/s ratio
...
void process (struct dt_iop_module_t *self, dt_dev_pixelpipe_iop_t *piece, void *ivoid, void *ovoid, const dt_iop_roi_t *roi_in, const dt_iop_roi_t *roi_out)
{
  ...
  for(int j=0;j<roi_out->height;j++) for(int i=0;i<roi_out->width;i++)
  {
    ...
    {
      double ox=cosv*2;    // scale outer vignette circle
      double oy=sinv*2;
      double blen=sqrt(((vv.x-ox)*(vv.x-ox))+((vv.y-oy)*(vv.y-oy)));             // lenght between vv and outer circle
      weight=((cplen-cvlen)/blen)*(data->strength*4);
    }
...
void init(dt_iop_module_t *module)
{
  ...
  dt_iop_vignette_params_t tmp = (dt_iop_vignette_params_t){.8,.5,.0,0,{0,0}};
...
void gui_init(struct dt_iop_module_t *self)
{
  ...
  g->label4 = GTK_LABEL(gtk_label_new(_("bsratio")));
  ...
  gtk_object_set(GTK_OBJECT(g->scale4), "tooltip-text", _("brightness/saturation radtio\nof the result,\n-1 - Only brightness\n 0 - 50/50 mix of brightness and saturation\n+1 - Only saturation"), NULL);
