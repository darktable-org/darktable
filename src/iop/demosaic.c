...
#define FDC_FILTERS   static const float complex Minv[3][8] = {                                                 \
...
#define TRANSLATE(n, size) ((n >= size) ? (2 * size - n - 2) : abs(n))
      ...
      for(int row = pad_fdc; row < mrow - pad_fdc; row++)
        ...
        {
          int myrow, mycol, fdc_row, fdc_col;
          uint8_t hm[8] = { 0 };
          uint8_t maxval = 0;
          for(int d = 0; d < ndir; d++)
          {
...
#if defined(__SSE__)
#define TS 122
static void xtrans_fdc_interpolate_sse2(struct dt_iop_module_t *self, float *out, const float *const in,
                                        const dt_iop_roi_t *const roi_out, const dt_iop_roi_t *const roi_in,
                                        const uint8_t (*const xtrans)[6])
{
  ...
  // filter preparation
  __m128 *const hptr = (__m128*)dt_alloc_align(16, 1664 * sizeof(__m128));
  if(!hptr)
  {
    fprintf(stderr, "[demosaic] not able to allocate FDC filter buffers\n");
    return;
  }
  for(int offset = 0; offset < 4; offset++)
  {
    __m128 *filptr = hptr + offset * 416;
    for(int filnum = 0; filnum < 8; filnum++)
      for(int fdc_row = 0; fdc_row < 13; fdc_row++)
      {
        __m128 filter;
        int start_num = 0;
        float filter_elem[4];
        ...
        for(int c = start_num; c < start_num + 13; c++)
        {
          filter_elem[c-block*4] = (filnum % 2 == 0) ? crealf(harr[(int)(filnum/2)][12-fdc_row][12-(c-offset)]) :
            cimagf(harr[(int)(filnum/2)][12-fdc_row][12-(c-offset)]);
          if((c + 1) % 4 == 0)
          {
            filter = _mm_set_ps(filter_elem[3],filter_elem[2],filter_elem[1],filter_elem[0]);
            *(filptr + 52 * filnum + 4 * fdc_row + block) = filter;
            block++;
          }
        }
        if(offset != 3)
        {
          for(int c = 1; c < 4 - offset; c++) filter_elem[4-c] = 0.0f;
          filter = _mm_set_ps(filter_elem[3],filter_elem[2],filter_elem[1],filter_elem[0]);
          *(filptr + 52 * filnum + 4 * fdc_row + block) = filter;
        }
      }
...
#define TRANSLATE(n, size) ((n >= size) ? (2 * size - n - 2) : abs(n))
      ...
      const int pad_fdc = 6;
      for(int row = pad_fdc; row < mrow - pad_fdc; row++)
        for(int col = pad_fdc, offset = 2; col < mcol - pad_fdc; col++, offset++)
        {
          int myrow, mycol, fdc_row, fdc_col;
          uint8_t hm[8] = { 0 };
          uint8_t maxval = 0;
          ...
          __m128 *filterptr;
          float preresults[4] __attribute__((aligned(16)));
          float results[8];
          // perform the convolution filtering using SSE2
          filterptr = hptr + offset * 416;
          for(int filnum = 0; filnum < 8; filnum++)
          {
            acc = _mm_setzero_ps();
            for(fdc_row = 0, myrow = row - 6; fdc_row < 13; fdc_row++, myrow++)
              for(fdc_col = 0, mycol = col - 6 - offset; fdc_col < 4; fdc_col++, mycol+=4)
              {
                datablock = _mm_load_ps(i_src + TS * myrow + mycol);
                kernel = *(filterptr + 52 * filnum + 4 * fdc_row + fdc_col);
                prod = _mm_mul_ps(kernel, datablock);
                acc = _mm_add_ps(acc, prod);
              }
            _mm_storeu_ps(&preresults[0], acc);
            results[filnum] = preresults[0] + preresults[1] + preresults[2] + preresults[3];
          }
          C2m = results[0] + results[1] * _Complex_I;
          C5m = results[2] + results[3] * _Complex_I;
          C7m = results[4] + results[5] * _Complex_I;
          C10m = results[6] + results[7] * _Complex_I;
          // build the q vector components
          myrow = (row + rowoffset) % 6;
          mycol = (col + coloffset) % 6;
          float complex modulator[8];
...
static void process_internal(struct dt_iop_module_t *self, dt_dev_pixelpipe_iop_t *piece, const void *const i, void *const o,
                             const dt_iop_roi_t *const roi_in, const dt_iop_roi_t *const roi_out, const int use_sse)
{
  ...
  if(qual_flags & DEMOSAIC_FULL_SCALE)
  {
    ...
    {
      if(demosaicing_method == DT_IOP_DEMOSAIC_FDC && (qual_flags & DEMOSAIC_XTRANS_FULL) && use_sse)
        xtrans_fdc_interpolate_sse2(self, tmp, pixels, &roo, &roi, xtrans);
...
{
  process_internal(self, piece, i, o, roi_in, roi_out, 0);
}
...
void process_sse2(struct dt_iop_module_t *self, dt_dev_pixelpipe_iop_t *piece, const void *const i, void *const o,
                  const dt_iop_roi_t *const roi_in, const dt_iop_roi_t *const roi_out)
{
  process_internal(self, piece, i, o, roi_in, roi_out, 1);
}
#endif
