...
typedef enum dt_iop_colorbalancergb_color_wheel_cursor_t
{
  CURSOR_NONE = 1 << 0,
  CURSOR_SHADOWS = 1 << 1,
  CURSOR_MIDTONES = 1 << 2,
  CURSOR_HIGHLIGHTS = 1 << 3,
} dt_iop_colorbalancergb_color_wheel_cursor_t;

// DisplayValue = A + B * exp(C * SliderValue);
// SliderValue = log((DisplayValue - A) / B) / C; => 0 to 1 value
// Compute power curve scale
// f(0.0) = x
// f(0.5) = y
// f(1.0) = z
typedef struct
{
  double x, y, z;
  double A, B, C;
} dt_iop_colorbalancergb_non_linear_scaling_t;

typedef struct dt_iop_colorbalancergb_color_wheel_data_t
{
  int selected_cursor, hovered_cursor;
...
typedef struct dt_iop_colorbalancergb_gui_data_t
{
  ...
  dt_iop_colorbalancergb_color_wheel_data_t color_wheel_data;
...
static void YchToRGB(dt_aligned_pixel_t *RGB_out, float chroma, float hue,
                                ...
                                GtkWidget *w, const float hue)
{
  ...
  for(int i = 0; i < DT_BAUHAUS_SLIDER_MAX_STOPS; i++)
  {
    ...
    YchToRGB(&RGB, x, h, output_profile, output_matrix_LMS_to_RGB);
...
static dt_iop_colorbalancergb_non_linear_scaling_t non_linear_scaling_init(float x, float y, float z)
{
  const double A = (x * z - y * y) / (x - 2 * y + z);
  const double B = (y - x) * (y - x) / (x - 2 * y + z);
  const double C = 2.0 * log((z - y) / (y - x));
  dt_iop_colorbalancergb_non_linear_scaling_t s = { .x = x, .y = y, .z = z, .A = A, .B = B, .C = C };
  return s;
}

static inline double non_linear_scaling_display_value(const dt_iop_colorbalancergb_non_linear_scaling_t *self, double slider_value)
{
  return self->A + self->B * exp(self->C * slider_value);
}

static inline double non_linear_scaling_slider_value(const dt_iop_colorbalancergb_non_linear_scaling_t *self, double display_value)
{
  return log((display_value - self->A) / self->B) / self->C;
}

static void paint_color_wheel_hues(cairo_t *cr, const dt_iop_colorbalancergb_gui_data_t *gui_data)
{
  if (gui_data->sliders_output_profile == NULL)
    return;
  ...
  // Arcs shows hues with (10 degrees increments)
  // And chroma changes
  const dt_iop_colorbalancergb_color_wheel_data_t *cw = &gui_data->color_wheel_data;

  const double degreesInc = 360.0f / cw->hueSteps;
  const double radInc = degreesInc * M_PI / 180.0;
  const double stroke_width = DT_PIXEL_APPLY_DPI(cw->radius / cw->chromaSteps);
  cairo_set_line_width(cr, stroke_width);
  cairo_set_antialias(cr, CAIRO_ANTIALIAS_GOOD);

  const float chromaStep = 1.0f / cw->chromaSteps;
  ...
  prepare_RGB_Yrg_matrices(gui_data->sliders_output_profile, input_matrix, output_matrix);

  // Color Wheel
  for(int j = 0; j < cw->chromaSteps; ++j)
  {
    const double chroma = non_linear_scaling_display_value( &cw->chromaScaling, chromaStep * (j + 1));
    const double radius = cw->radius / cw->chromaSteps * (j + 1) - stroke_width / 2;
    for(int i = 0, m = (int)360.0 / degreesInc; i < m; ++i)
    {
      // Color from Ych to RGB
      const float hue = DEG_TO_RAD(i * degreesInc);
      dt_aligned_pixel_t RGB;
      YchToRGB(&RGB, chroma, hue, gui_data->sliders_output_profile, output_matrix);
      cairo_set_source_rgb(cr, RGB[0], RGB[1], RGB[2]);
      const double antialiasingMargin = 0.01;
      cairo_arc(cr,
                cw->centerX,
                cw->centerY,
                radius,
                i * radInc - M_PI_2 - antialiasingMargin,
                (i + 1) * radInc - M_PI_2 + antialiasingMargin);
      cairo_stroke(cr);
    }
  }
  ...
  cairo_arc(cr, cw->centerX, cw->centerY, cw->radius, 0, 2 * M_PI);
  cairo_stroke(cr);
}

typedef struct dt_iop_colorbalancergb_color_wheel_cursor_position_t
{
  float x;
  float y;
} dt_iop_colorbalancergb_color_wheel_cursor_position_t;

static dt_iop_colorbalancergb_color_wheel_cursor_position_t get_color_wheel_cursor_position(
                                     double hue,
                                     double chroma,
                                     const dt_iop_colorbalancergb_color_wheel_data_t *cw)
{
  const float distanceToWheelCenter
      = non_linear_scaling_slider_value(&cw->chromaScaling,
                                        fmaxf(fminf(chroma, cw->chromaScaling.z), cw->chromaScaling.x))
        * cw->radius;
  const float hueAngle = hue * M_PI / 180.0 - M_PI_2;

  return (dt_iop_colorbalancergb_color_wheel_cursor_position_t) {
    .x = cw->centerX + cos(hueAngle) * distanceToWheelCenter,
    .y = cw->centerY + sin(hueAngle) * distanceToWheelCenter
  };
}

static void hue_and_chroma_for_cursor_position(int width, int height,
                                               float x, float y,
                                               float *hue, float *chroma,
                                               const dt_iop_colorbalancergb_color_wheel_data_t *cw)
{
  const float xC = x - cw->centerX;
  const float yC = y - cw->centerY;
  const float hueAngle = atan2f(yC, xC); // in [-PI, PI]

  // Control locks the chroma
  if (!(cw->modifiers_state & GDK_CONTROL_MASK)) {
    const float distanceToWheelCenter = sqrt(xC * xC + yC * yC);
    float normalizedDistance = fminf(distanceToWheelCenter, cw->radius) / cw->radius;

    // Round chroma to fixed step if shift pressed
    if(cw->modifiers_state & GDK_SHIFT_MASK) {
      normalizedDistance = roundf(normalizedDistance / 0.1f) * 0.1f;  // Round to nearest 0.1
    }
    *chroma = non_linear_scaling_display_value(&cw->chromaScaling, normalizedDistance);
  }

  *hue = fmodf((hueAngle + M_PI_2) * 180.0f / M_PI + 360.0f, 360.0f); // [0, 360]

  // Round to nearest multiple of 10 if shift pressed
  if(cw->modifiers_state & GDK_SHIFT_MASK) {
    *hue = roundf(*hue / 10.0f) * 10.0f; // Round to nearest 10
  }
}

static void paint_color_wheel_cursor(cairo_t *cr,
                                     float greyLevel,
                                     float cursorRadius,
                                     double hue,
                                     double chroma,
                                     const dt_iop_colorbalancergb_color_wheel_data_t *cw)
{
  const dt_iop_colorbalancergb_color_wheel_cursor_position_t cursor_pos = get_color_wheel_cursor_position(hue, chroma, cw);

  cairo_set_source_rgba(cr, greyLevel, greyLevel, greyLevel, 0.6);
  cairo_arc(cr, cursor_pos.x, cursor_pos.y, cursorRadius, 0, 2 * M_PI);
  cairo_fill(cr);
  ...
  cairo_save(cr);
  cairo_set_line_width(cr, DT_PIXEL_APPLY_DPI(1));
  cairo_set_source_rgb(cr, greyLevel, greyLevel, greyLevel);
  cairo_move_to(cr, cw->centerX, cw->centerY);
  cairo_line_to(cr, cursor_pos.x, cursor_pos.y);
  cairo_stroke(cr);
  cairo_restore(cr);
}
...
static void paint_color_wheel(GtkWidget *widget, cairo_t *crf, gpointer user_data)
{
  ...
  const dt_iop_colorbalancergb_gui_data_t *g = (dt_iop_colorbalancergb_gui_data_t *)self->gui_data;
  const dt_iop_colorbalancergb_color_wheel_data_t *cw = &g->color_wheel_data;

  // Background hues
  paint_color_wheel_hues(cr, g);
  // Highlight Cursor
  paint_color_wheel_cursor(cr, 1.0, cw->hovered_cursor == CURSOR_HIGHLIGHTS ? 8.0f : 4.0f, p->highlights_H, p->highlights_C, cw);
  // Shadow Cursor
  paint_color_wheel_cursor(cr, 0.2, cw->hovered_cursor == CURSOR_SHADOWS ? 8.0f : 4.0f, p->shadows_H, p->shadows_C, cw);
  // Midtone Cursor
  paint_color_wheel_cursor(cr, 0.6, cw->hovered_cursor == CURSOR_MIDTONES ? 8.0f : 4.0f, p->midtones_H, p->midtones_C, cw);
...
static inline gboolean color_wheel_mouse_hovers_cursor(float mouse_x, float mouse_y, const dt_iop_colorbalancergb_color_wheel_cursor_position_t *cursor_pos)
{
  const float dist_sqr = (cursor_pos->x - mouse_x) * (cursor_pos->x - mouse_x) + (cursor_pos->y - mouse_y) * (cursor_pos->y - mouse_y);
  // Allow a radius of 16 px
  return dist_sqr <= 16;
}

static dt_iop_colorbalancergb_color_wheel_cursor_t color_wheel_hovered_cursor(float mouse_x, float mouse_y, const dt_iop_module_t *self)
{
  const dt_iop_colorbalancergb_gui_data_t *g = (const dt_iop_colorbalancergb_gui_data_t *)self->gui_data;
  const dt_iop_colorbalancergb_params_t *p = (const dt_iop_colorbalancergb_params_t *)self->params;

  const dt_iop_colorbalancergb_color_wheel_cursor_position_t highlights_pos =
  get_color_wheel_cursor_position(p->highlights_H, p->highlights_C, &g->color_wheel_data);
  if (color_wheel_mouse_hovers_cursor(mouse_x, mouse_y, &highlights_pos))
    return CURSOR_HIGHLIGHTS;

  const dt_iop_colorbalancergb_color_wheel_cursor_position_t midtones_pos =
  get_color_wheel_cursor_position(p->midtones_H, p->midtones_C, &g->color_wheel_data);
  if (color_wheel_mouse_hovers_cursor(mouse_x, mouse_y, &midtones_pos))
    return CURSOR_MIDTONES;

  const dt_iop_colorbalancergb_color_wheel_cursor_position_t shadows_pos =
  get_color_wheel_cursor_position(p->shadows_H, p->shadows_C, &g->color_wheel_data);
  if (color_wheel_mouse_hovers_cursor(mouse_x, mouse_y, &shadows_pos))
    return CURSOR_SHADOWS;
...
static gboolean color_wheel_mouse_move(GtkWidget *widget, GdkEventMotion *event, gpointer user_data)
{
  dt_iop_module_t *self = (dt_iop_module_t *)user_data;
  dt_iop_colorbalancergb_gui_data_t *g = (dt_iop_colorbalancergb_gui_data_t *)self->gui_data;
  dt_iop_colorbalancergb_params_t *p = (dt_iop_colorbalancergb_params_t *)self->params;

  static float *chroma = NULL;
  static float *hue = NULL;
  static GtkWidget *chroma_slider = NULL;
  static GtkWidget *hue_slider = NULL;
  static float last_hue = 0.0f;
  static float last_chroma = 0.0f;
  ...
  if (g->color_wheel_data.selected_cursor != CURSOR_NONE) {
    if (chroma == NULL) {
      switch(g->color_wheel_data.selected_cursor) {
        case CURSOR_SHADOWS: {
          chroma = &p->shadows_C;
          hue = &p->shadows_H;
          chroma_slider = g->shadows_C;
          hue_slider = g->shadows_H;
          break;
        }
        case CURSOR_MIDTONES: {
          chroma = &p->midtones_C;
          hue = &p->midtones_H;
          chroma_slider = g->midtones_C;
          hue_slider = g->midtones_H;
          break;
        }
        case CURSOR_HIGHLIGHTS: {
          chroma = &p->highlights_C;
          hue = &p->highlights_H;
          chroma_slider = g->highlights_C;
          hue_slider = g->highlights_H;
          break;
        }
        default: break;
      }
    ...
    assert(chroma != NULL && hue != NULL && chroma_slider != NULL && hue_slider != NULL);

    // Fair to assure that selected and hovered cursor are the same
    g->color_wheel_data.hovered_cursor = g->color_wheel_data.selected_cursor;
    g->color_wheel_data.modifiers_state = event->state;

    GtkAllocation allocation;
    gtk_widget_get_allocation(widget, &allocation);
    const int width = allocation.width, height = allocation.height;
    hue_and_chroma_for_cursor_position(width, height, event->x, event->y, hue, chroma, &g->color_wheel_data);

    // Will trigger a color wheel redraw
    if (*chroma != last_chroma) {
      dt_bauhaus_slider_set(chroma_slider, *chroma);
      last_chroma = *chroma;
      gui_changed(self, chroma_slider, NULL);
    }
    if (*hue != last_hue) {
      dt_bauhaus_slider_set(hue_slider, *hue);
      last_hue = *hue;
      gui_changed(self, hue_slider, NULL);
    }
    ...
    chroma_slider = NULL;
    hue_slider = NULL;
    g->color_wheel_data.hovered_cursor = color_wheel_hovered_cursor(event->x, event->y, self);
    g->color_wheel_data.modifiers_state = 0;
    gui_changed(self, GTK_WIDGET(g->color_wheel_area), NULL);
  }
  return FALSE;
}
...
static gboolean color_wheel_mouse_scroll(GtkWidget *widget, GdkEventScroll *event, gpointer user_data)
{
  ...
  const float radiuses[3] = {
      non_linear_scaling_slider_value(&cw->chromaScaling,
                                      fmaxf(fminf(p->shadows_C, cw->chromaScaling.z), cw->chromaScaling.x)) * cw->radius,
      non_linear_scaling_slider_value(&cw->chromaScaling,
                                      fmaxf(fminf(p->midtones_C, cw->chromaScaling.z), cw->chromaScaling.x)) * cw->radius,
      non_linear_scaling_slider_value(&cw->chromaScaling,
                                      fmaxf(fminf(p->highlights_C, cw->chromaScaling.z), cw->chromaScaling.x)) * cw->radius,
  };
                              ...
                              const dt_iop_colorbalancergb_gui_data_t *const g)
{
  for(int i = 0; i < DT_BAUHAUS_SLIDER_MAX_STOPS; i++)
  {
    ...
    dt_aligned_pixel_t RGB;
    YchToRGB(&RGB, MIN(0.2f, max_chroma), h, output_profile, output_matrix_LMS_to_RGB);
...
void gui_changed(dt_iop_module_t *self, GtkWidget *w, void *previous)
{
  ...
  if (!w ||
       w == g->highlights_H || w == g->highlights_C ||
       w == g->midtones_H || w == g->midtones_C ||
       w == g->shadows_H || w == g->shadows_C ||
       w == GTK_WIDGET(g->color_wheel_area) ||
       output_profile_changed)
    gtk_widget_queue_draw(GTK_WIDGET(g->color_wheel_area));
...
void gui_init(dt_iop_module_t *self)
{
  ...
  // Color Wheel
  g->color_wheel_area = GTK_DRAWING_AREA(dtgtk_drawing_area_new_with_aspect_ratio(1.0));
  g_object_set_data(G_OBJECT(g->color_wheel_area), "iop-instance", self);
  dt_action_define_iop(self, NULL, N_("colorwheel"), GTK_WIDGET(g->color_wheel_area), NULL);
  g_signal_connect(G_OBJECT(g->color_wheel_area), "draw", G_CALLBACK(paint_color_wheel), self);
  g_signal_connect(G_OBJECT(g->color_wheel_area), "realize", G_CALLBACK(init_color_wheel), self);
  g_signal_connect(G_OBJECT(g->color_wheel_area), "motion-notify-event", G_CALLBACK(color_wheel_mouse_move), self);
  g_signal_connect(G_OBJECT(g->color_wheel_area), "button-press-event", G_CALLBACK(color_wheel_mouse_button_press_release), self);
  g_signal_connect(G_OBJECT(g->color_wheel_area), "button-release-event", G_CALLBACK(color_wheel_mouse_button_press_release), self);
  g_signal_connect(G_OBJECT(g->color_wheel_area), "scroll-event", G_CALLBACK(color_wheel_mouse_scroll), self);

  gtk_box_pack_start(GTK_BOX(self->widget), GTK_WIDGET(g->color_wheel_area), FALSE, FALSE, 0);
