...
static inline double non_linear_scaling_display_value(const dt_iop_colorbalancergb_non_linear_scaling_t *self,
                                                      double slider_value)
{
  return self->A + self->B * exp(self->C * slider_value);
}

static inline double non_linear_scaling_slider_value(const dt_iop_colorbalancergb_non_linear_scaling_t *self,
...
{
  if(gui_data->sliders_output_profile == NULL) return;
  ...
  for(int j = 0; j < cw->chromaSteps; ++j)
  {
    ...
    for(int i = 0, m = (int)360.0 / degreesInc; i < m; ++i)
    {
      ...
      cairo_arc(cr, cw->centerX, cw->centerY, radius, i * radInc - M_PI_2 - antialiasingMargin,
...
static dt_iop_colorbalancergb_color_wheel_cursor_position_t
get_color_wheel_cursor_position(double hue, double chroma, const dt_iop_colorbalancergb_color_wheel_data_t *cw)
{
...
static void hue_and_chroma_for_cursor_position(int width, int height, float x, float y, float *hue, float *chroma,
...
static void paint_color_wheel_cursor(cairo_t *cr, float greyLevel, float cursorRadius, double hue, double chroma,
                                     const dt_iop_colorbalancergb_color_wheel_data_t *cw)
{
  const dt_iop_colorbalancergb_color_wheel_cursor_position_t cursor_pos
      = get_color_wheel_cursor_position(hue, chroma, cw);
...
static void paint_color_wheel(GtkWidget *widget, cairo_t *crf, gpointer user_data)
{
  ...
  // Highlight Cursor
  paint_color_wheel_cursor(cr, 1.0, cw->hovered_cursor == CURSOR_HIGHLIGHTS ? 8.0f : 4.0f, p->highlights_H,
                           p->highlights_C, cw);
  // Shadow Cursor
  paint_color_wheel_cursor(cr, 0.2, cw->hovered_cursor == CURSOR_SHADOWS ? 8.0f : 4.0f, p->shadows_H, p->shadows_C,
                           cw);
  // Midtone Cursor
  paint_color_wheel_cursor(cr, 0.6, cw->hovered_cursor == CURSOR_MIDTONES ? 8.0f : 4.0f, p->midtones_H,
                           p->midtones_C, cw);
...
color_wheel_mouse_hovers_cursor(float mouse_x, float mouse_y,
                                const dt_iop_colorbalancergb_color_wheel_cursor_position_t *cursor_pos)
{
  const float dist_sqr = (cursor_pos->x - mouse_x) * (cursor_pos->x - mouse_x)
                         + (cursor_pos->y - mouse_y) * (cursor_pos->y - mouse_y);
  // Allow a radius of 16 px
  return dist_sqr <= 16;
}

static dt_iop_colorbalancergb_color_wheel_cursor_t color_wheel_hovered_cursor(float mouse_x, float mouse_y,
                                                                              const dt_iop_module_t *self)
{
  ...
  if(color_wheel_mouse_hovers_cursor(mouse_x, mouse_y, &highlights_pos)) return CURSOR_HIGHLIGHTS;
  ...
  if(color_wheel_mouse_hovers_cursor(mouse_x, mouse_y, &midtones_pos)) return CURSOR_MIDTONES;
  ...
  if(color_wheel_mouse_hovers_cursor(mouse_x, mouse_y, &shadows_pos)) return CURSOR_SHADOWS;
...
static gboolean color_wheel_mouse_scroll(GtkWidget *widget, GdkEventScroll *event, gpointer user_data)
{
  ...
  const float radiuses[3] = {
    non_linear_scaling_slider_value(&cw->chromaScaling,
        ...
        * cw->radius,
    non_linear_scaling_slider_value(&cw->chromaScaling,
                                    fmaxf(fminf(p->midtones_C, cw->chromaScaling.z), cw->chromaScaling.x))
        * cw->radius,
    non_linear_scaling_slider_value(&cw->chromaScaling,
                                    fmaxf(fminf(p->highlights_C, cw->chromaScaling.z), cw->chromaScaling.x))
        * cw->radius,
  };
...
void gui_changed(dt_iop_module_t *self, GtkWidget *w, void *previous)
{
  ...
  if(!w || w == g->highlights_H || w == g->highlights_C || w == g->midtones_H || w == g->midtones_C
     || w == g->shadows_H || w == g->shadows_C || w == GTK_WIDGET(g->color_wheel_area) || output_profile_changed)
...
void gui_init(dt_iop_module_t *self)
{
  ...
  g_signal_connect(G_OBJECT(g->color_wheel_area), "button-press-event",
                   G_CALLBACK(color_wheel_mouse_button_press_release), self);
  g_signal_connect(G_OBJECT(g->color_wheel_area), "button-release-event",
                   G_CALLBACK(color_wheel_mouse_button_press_release), self);
