...
static inline float _calc_vignette_spline(const float radius, const float *spline)
{
               ...
               const dt_iop_roi_t *const roi_out)
{
  ...
  else
  {
    ...
    err = dt_opencl_enqueue_copy_image(piece->pipe->devid, data, dev_out, origin, origin, region);
  }
