...
void gui_update(struct dt_iop_module_t *self)
{
  ...
  dtgtk_slider_set_value(g->scale2, -log2f(p->white/self->dev->image->maximum));
...
white_callback (GtkDarktableSlider *slider, gpointer user_data)
{
  ...
  dt_iop_exposure_params_t *p = (dt_iop_exposure_params_t *)self->params;
  p->white = exp2f(-dtgtk_slider_get_value(slider))*self->dev->image->maximum;
  float black = dtgtk_slider_get_value(g->scale1);
  if(p->white < black) dtgtk_slider_set_value(g->scale1, p->white);
...
expose (GtkWidget *widget, GdkEventExpose *event, dt_iop_module_t *self)
{
  ...
  dt_iop_exposure_params_t *p = (dt_iop_exposure_params_t *)self->params;
  dt_iop_exposure_gui_data_t *g = (dt_iop_exposure_gui_data_t *)self->gui_data;
  p->white = fmaxf(fmaxf(self->picked_color_max[0], self->picked_color_max[1]), self->picked_color_max[2]) * (1.0-dtgtk_slider_get_value(DTGTK_SLIDER(g->autoexpp)));
...
void gui_init(struct dt_iop_module_t *self)
{
  ...
  g->scale2 = DTGTK_SLIDER(dtgtk_slider_new_with_range( DARKTABLE_SLIDER_VALUE, -3.0, 6.0, .02,-log2f(p->white/self->dev->image->maximum),3));
