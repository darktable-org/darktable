...
DT_MODULE_INTROSPECTION(1, dt_iop_cropping_params_t)

typedef struct dt_iop_cropping_aspect_t
{
  char *name;
  int d, n;
} dt_iop_cropping_aspect_t;

typedef struct dt_iop_cropping_params_t
{
...
typedef struct dt_iop_cropping_gui_data_t
{
...
} dt_iop_cropping_gui_data_t;

typedef struct dt_iop_cropping_data_t
{
...
{
  return dt_iop_set_description(self, _("change the framing"),
...
#pragma omp parallel for simd default(none) \
    dt_omp_firstprivate(points_count, points, d, factor) \
...
{
  dt_iop_cropping_gui_data_t *g = (dt_iop_cropping_gui_data_t *)self->gui_data;
  dt_iop_cropping_params_t *p = (dt_iop_cropping_params_t *)self->params;
                   ...
                   const dt_iop_roi_t *roi_out, dt_iop_roi_t *roi_in)
{
  ...
  *roi_in = *roi_out;
  ...
  // sanity check.
  const float scwidth = piece->buf_in.width * so;
  const float scheight = piece->buf_in.height * so;
...
{
  dt_iop_cropping_params_t *p = (dt_iop_cropping_params_t *)p1;
  dt_iop_cropping_data_t *d = (dt_iop_cropping_data_t *)piece->data;
...
{
  piece->data = malloc(sizeof(dt_iop_cropping_data_t));
}
...
void reload_defaults(dt_iop_module_t *self)
{
  ...
  dt_iop_cropping_params_t *d = (dt_iop_cropping_params_t *)self->default_params;
...
static void aspect_presets_changed(GtkWidget *combo, dt_iop_module_t *self)
{
  ...
  else
  {
    ...
    {
      const dt_iop_cropping_aspect_t *aspect = iter->data;
  ...
  if(d != abs(p->ratio_d) || n != p->ratio_n)
  {
    ...
    p->ratio_n = n;
    dt_conf_set_int("plugins/darkroom/cropping/ratio_d", abs(p->ratio_d));
    dt_conf_set_int("plugins/darkroom/cropping/ratio_n", abs(p->ratio_n));
...
//  dt_iop_cropping_gui_data_t *g = (dt_iop_cropping_gui_data_t *)self->gui_data;
  ...
  dt_conf_set_int("plugins/darkroom/cropping/ratio_d", 0);
  dt_conf_set_int("plugins/darkroom/cropping/ratio_n", 0);
}
...
void gui_update(struct dt_iop_module_t *self)
{
  ...
  {
    p->ratio_d = dt_conf_get_int("plugins/darkroom/cropping/ratio_d");
    p->ratio_n = dt_conf_get_int("plugins/darkroom/cropping/ratio_n");
  }
...
static void guides_presets_set_visibility(dt_iop_cropping_gui_data_t *g, int which)
{
...
static void guides_presets_changed(GtkWidget *combo, dt_iop_module_t *self)
{
  ...
  // remember setting
  dt_conf_set_int("plugins/darkroom/cropping/guide", which);

  dt_control_queue_redraw_center();
}

static gint _aspect_ratio_cmp(const dt_iop_cropping_aspect_t *a, const dt_iop_cropping_aspect_t *b)
{
...
{
  dt_iop_cropping_gui_data_t *g = IOP_GUI_ALLOC(cropping);
  ...
  dt_iop_cropping_aspect_t aspects[] = { { _("freehand"), 0, 0 },
  ...
  const int aspects_count = sizeof(aspects) / sizeof(dt_iop_cropping_aspect_t);

  for(int i = 0; i < aspects_count; i++)
  {
    dt_iop_cropping_aspect_t *aspect = g_malloc(sizeof(dt_iop_cropping_aspect_t));
  ...
  int d = ((dt_iop_cropping_aspect_t *)g->aspect_list->data)->d + 1,
      n = ((dt_iop_cropping_aspect_t *)g->aspect_list->data)->n + 1;
  for(GList *iter = g->aspect_list; iter; iter = g_list_next(iter))
  {
    dt_iop_cropping_aspect_t *aspect = (dt_iop_cropping_aspect_t *)iter->data;
  ...
  int guide = dt_conf_get_int("plugins/darkroom/cropping/guide");
...
{
  dt_iop_cropping_aspect_t *aspect = (dt_iop_cropping_aspect_t *)data;
  g_free(aspect->name);
  aspect->name = NULL;
  g_free(aspect);
}
...
static _grab_region_t get_grab(float pzx, float pzy, dt_iop_cropping_gui_data_t *g, const float border,
