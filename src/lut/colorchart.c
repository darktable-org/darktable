...
void free_chart(chart_t *chart)
{
  ...
  if(chart->d_table)
    g_hash_table_unref(chart->d_table);
  if(chart->box_table)
    g_hash_table_unref(chart->box_table);
  if(chart->patch_sets)
    g_hash_table_unref(chart->patch_sets);
  free(chart);
}
...
#define ERROR {lineno = __LINE__; goto error;}
...
chart_t* parse_cht(const char* filename)
{
  ...
  while(fgets(line, MAX_LINE_LENGTH, fp))
  {
    ...
    if(!g_strcmp0(keyword, "BOXES") && last_block < BLOCK_BOXES)
    {
      ...
      while(fgets(line, MAX_LINE_LENGTH, fp))
      {
        ...
        if(*c == 'F')
        {
          ...
          // if(sscanf(line, " F _ _ %f %f %f %f %f %f %f %f", &x0, &y0, &x1, &y1, &x2, &y2, &x3, &y3) != 8) ERROR;
          c++;
          while(*c == ' ') c++;
          if(*c++ != '_') ERROR;
          while(*c == ' ') c++;
        ...
        else if((*c == 'D') || (*c == 'X') || (*c == 'Y'))
        {
          ...
          *c++ = '\0';
          ...
          size_t lxs_len = strlen(lxs),
          lxe_len = strlen(lxe),
          lys_len = strlen(lys),
          lye_len = strlen(lye);
          if(lxs_len > lxe_len || lys_len > lye_len) ERROR;

          char x_label[lxe_len + 1], y_label[lye_len + 1]; // make sure there is enough room to add another char in the beginning
      ...
      // so let's scale all the values to have a bounding box with the longer side having length 1 and start at (0, 0)
    ...
    else if(!g_strcmp0(keyword, "EXPECTED") && last_block < BLOCK_EXPECTED)
    {
      ...
      if(!g_strcmp0(cs, "XYZ"))      color_space = DT_COLORSPACE_XYZ;
      else if(!g_strcmp0(cs, "LAB")) color_space = DT_COLORSPACE_LAB;
      else ERROR;
...
int parse_it8(const char *filename, chart_t *chart)
{
  ...
  dt_colorspaces_color_profile_type_t color_space = DT_COLORSPACE_NONE;
  int column_SAMPLE_ID = -1, column_X = -1, column_Y = -1, column_Z = -1, column_L = -1, column_a = -1, column_b = -1;
  ...
  {
    if(!g_strcmp0(sample_names[i], "SAMPLE_ID"))  column_SAMPLE_ID = i;
    else if(!g_strcmp0(sample_names[i], "XYZ_X")) column_X = i;
    else if(!g_strcmp0(sample_names[i], "XYZ_Y")) column_Y = i;
    else if(!g_strcmp0(sample_names[i], "XYZ_Z")) column_Z = i;
    else if(!g_strcmp0(sample_names[i], "LAB_L")) column_L = i;
    else if(!g_strcmp0(sample_names[i], "LAB_A")) column_a = i;
    else if(!g_strcmp0(sample_names[i], "LAB_B")) column_b = i;
  }
  ...
  while(g_hash_table_iter_next(&table_iter, &key, &value))
  {
    ...
    set_color(box, color_space,
              cmsIT8GetDataDbl(hIT8, key, columns[0]),
              cmsIT8GetDataDbl(hIT8, key, columns[1]),
