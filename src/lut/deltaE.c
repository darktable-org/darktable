...
float dt_colorspaces_deltaE_2000(float Lab0[3], float Lab1[3])
{
  ...
  float H_ip;
  if(fabsf(h1_p - h2_p) > 180.0) H_ip = (h1_p + h2_p + 360.0) * 0.5;
  else                           H_ip = (h1_p + h2_p) * 0.5;
  float T    = 1.0
               - 0.17 * cosf(DEG2RAD(H_ip - 30.0))
               + 0.24 * cosf(DEG2RAD(2.0 * H_ip))
               + 0.32 * cosf(DEG2RAD(3.0 * H_ip + 6.0))
               - 0.20 * cosf(DEG2RAD(4.0 * H_ip - 63.0));
  float dh_p = h2_p - h1_p;
  if(fabsf(dh_p) > 180.0)
  {
    if(h2_p <= h1_p) dh_p += 360.0;
    else             dh_p -= 360.0;
  }
  ...
  float dE   = sqrtf(  (dL_p / (KL * SL)) * (dL_p / (KL * SL))
                     + (dC_p / (KC * SC)) * (dC_p / (KC * SC))
                     + (dH_p / (KH * SH)) * (dH_p / (KH * SH))
                     + RT * (dC_p / (KC * SC)) * (dH_p / (KH * SH))
                    );
  return dE;
}
