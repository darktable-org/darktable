...
denoiseprofile_precondition_v2(read_only image2d_t in, write_only image2d_t out, const int width, const int height,
                             const float4 sqrta, const float4 p, const float4 b, const float4 wb)
{
  const int x = get_global_id(0);
  const int y = get_global_id(1);
  ...
  float4 t = 2.0f * native_powr(fmax((float4)0.0f, pixel / wb) + b, 1.0f - p / 2.0f) / ((-p + 2.0f) * sqrta);
                             ...
                             const float4 a, const float4 p, const float4 b, const float4 wb)
{
  ...
  float4 delta = px * px + 4.0f * a0 * a1;
  float4 z = (px + sqrt(delta)) / (2.0f * a0);
