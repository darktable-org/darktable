...
C<darktable [options] [IMG_1234.RAW]>
...
and B<Mac OS X> ine the lines of B<Adobe Lightroom> and B<Apple Aperture>.
...
Most of processing is done in 32bit float per channel mode in device independent 
B<CIE L*a*b*> color space. B<darktable> is also fully color managed, which gives 
you full control over the look of the photos.

The application relies on modern plug-in architecture thus making it easy for 
