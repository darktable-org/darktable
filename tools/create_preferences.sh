...
function key_begin {
  ...
  g_signal_connect(G_OBJECT(labelev), "button-press-event", G_CALLBACK(double_click_callback), - $num);
...
function key_end {
  ...
  gtk_box_pack_start(GTK_BOX(vbox1), labelev);
  gtk_box_pack_start(GTK_BOX(vbox2), widget);
  g_signal_connect(G_OBJECT(labelev), "activated", G_CALLBACK(preferences_callback), $num);
...
init_tab_$tab (GtkWidget *tab)
{
  ...
  gtk_box_pack_start(GTK_BOX(hbox), vbox1);
  gtk_box_pack_start(GTK_BOX(hbox), vbox1);
  gtk_notebook_append_page(GTK_NOTEBOOK(tab), hbox);
...
grep -E '<key>|<type>|<default>|<short>|<long>|<schema>|</schema>' darktable.schemas.in |
      sed -e 's/^[ \t]*//;s/[ \t]*$//' |
      tr -d '\n' | sed -e 's/<\/schema>/#/g'  | tr '#' '\n' | grep -E "tab:$tab" |
...
do
  ...
  g_signal_connect(G_OBJECT(widget), "activated", G_CALLBACK(preferences_callback), $num);
EOF
  elif [ $type == "int" ]; then
  ...
  g_signal_connect(G_OBJECT(widget), "value-changed", G_CALLBACK(preferences_callback), $num);
EOF
  elif [ $type == "bool" ]; then
  ...
  g_signal_connect(G_OBJECT(widget), "toggled", G_CALLBACK(preferences_callback), $num);
EOF
...
cat $callbackfile > $outfile
rm $callbackfile

for tab in $tabs;
do
  cat init_$tab.c >> $outfile
  rm init_$tab.c
done # end for all tabs
