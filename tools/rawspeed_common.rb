...
module RawSpeedCommon
  ...
  MANUAL_MUNGERS = {
    ...
    ['Leica Camera AG', 'M8 Digital Camera'] => %w[LEICA M8]
  }.freeze
  ...
  def self.munge_make_model(make, model)
    makemodel = if MANUAL_MUNGERS[[make, model]]
                ...
                elsif make.split[0..1].join(' ').casecmp('KONICA MINOLTA').zero?
                  [make.split[1], model.strip]
                elsif make.split[0].casecmp('RICOH').zero? && model.split[0].casecmp('PENTAX').zero?
                  [model.split[0], model.split[1..-1].join.strip]
                elsif make.casecmp('KODAK').zero?
                  [make.upcase, model]
                else
                  [make.split[0], model.strip]
                end
    [makemodel[0].upcase, makemodel[1].upcase]
  end
