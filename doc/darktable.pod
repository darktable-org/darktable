...
B<darktable-faster> first copies the library to ram and then starts dt. On close, it is copied back, so even in case of crashes no data loss is anticipated.
